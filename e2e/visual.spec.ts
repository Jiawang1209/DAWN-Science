...
const SCREENS: { name: string; go: (page: Page) => Promise<void> }[] = [
  ...
  },
  {
    ...
    go: async (page) => {
      ...
      /**
       * **先点到「模型服务」那一块**（2026-08-12）。
       *
       * 设置改成了「左分类 / 右内容」（作者：*「看不出太大的层次」*），
       * 默认停在「外观」。这一屏要截的是**有内容的那一块**——
       * 一张只有主题选择的图看不出这次改了什么。
       */
      ...
      /**
       * **分类那一列上的四个数要等到具体的值**（2026-09-16 补等待，2026-09-17 改成断值）。
       *
       * 「Skills / 子 Agent / 插件 / MCP 服务器」行尾那几个计数各走一发 IPC
       * （`App.tsx` 那个 `名册代` effect）。不等它们，这一屏截到的是
       * 「有时有、有时无」的一帧——那正是本项目反复栽过的那种假报警：
       * 看着像图片对比不稳定，其实是一场普通的竞态。
       *
       * **它们不是「一直都在」的老债，来历各不相同**（2026-09-17 查实，
       * 上一版这条注释说反了）：
       * - 「插件」「MCP 服务器」这两个数**是这条分支 2026-09-16 才加的**
       *   （`41dbf13`，Task 1）。基线存于 `aeb7616`（2026-09-11），
       *   `41dbf13` 不是它的祖先——所以基线里那两行没有数字是**对的**，
       *   它们那时还不存在。（写「今天」会跟着读的人变，这里一律写日期。）
       * - 「Skills」从 5 变成 6，**不是渲染变了，是内容真的多了一个**：
       *   `a74ab7a`（main，作者日 2026-09-14 / 提交日 09-15）加了
       *   `skills/analyze-like-case/`。**从它落地到 09-16 被发现为止，
       *   这张基线一直在如实反映一件已经不成立的事，而没有人收到消息**——
       *   逐像素对比把一次真实的内容变更吞掉了。
       *
       * **所以这里断的是值，不是「有没有」。** 两个好处，缺一不可：
       * ① 截图**在构造上**不可能落在过渡态；
       * ② 哪天有人再加一个技能，报出来的是 `expected "6", got "7"` ——
    ...
    test(`${screen.name} · ${theme}`, async ({ dawn }) => {
      ...
      await expect(page).toHaveScreenshot(`${screen.name}-${theme}.png`, {
        ...
        mask: [
          ...
          /**
           * **工具调用的「已跑 …」是秒表**（②-B · R2）。跑完之后那个数也不稳定——
           * 它是这台机器这一次真实花掉的时间。同一条理由：
