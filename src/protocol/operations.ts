        ...
        .array(
          z
            .object({
              ...
              /**
               * 这条话在说什么（2026-09-01 终审）：`catalog` = 目录里挑不出模型（B8），`key` = 填 key 那一次验证的结果（B9）。
               * 向导给 B8 的话套一层「已保存，但还建不出可用的模型」；B9 的话本身就是整句，套了就是 provider 名出现两次、
               * 还把「key 错了」说成「目录没模型」。可选：老后端不给 = 按 B8 那样套。
               */
              kind: z.enum(["catalog", "key"]).optional(),
            })
            .strict(),
        )
        .optional(),
