    ...
    rows: allRows.map((row, ri) => new TableRow({
      children: (Array.isArray(row) ? row : []).map((cell, ci) => {
        ...
        return new TableCell({
          ...
          /* exactOptionalPropertyTypes：undefined 不能显式塞给可选属性，改条件展开（移植时的本地化改动） */
          ...(() => {
            const 底 = stripe ? (ri === 0 ? cellShading(STRIPE_HEADER_FILL) : ri % 2 === 0 ? cellShading(STRIPE_EVEN_FILL) : undefined) : undefined
            return 底 ? { shading: 底 } : {}
          })(),
...
async function docxCreate(params: DocxCreateInput): Promise<string> {
  ...
  const background = params.background_color
    ...
    : undefined
  const doc = new Document({ ...(background ? { background } : {}), sections: [{ children }] })
  ...
  if (params.watermark?.text) {
    buf = await applyWatermark(buf, {
      ...
      ...(params.watermark.color !== undefined ? { color: params.watermark.color } : {}),
      ...(params.watermark.opacity !== undefined ? { opacity: params.watermark.opacity } : {}),
    })
...
export const docx工具: Office工具定义[] = [
  {
    name: 'docx_create',
