...
/**
 * 数据工具真的挂上了吗（2026-08-14）。
 *
 * `tests/tools/data.test.ts` 有 9 条，**验的是工具对象本身**——
 * 而它们全绿的同时，这个工具可以根本没被交给模型。
...
describe("数据工具 · 接线", () => {
  ...
  it("**模型手上有 describe_dataset**", () => {
    expect(工具名(new NativeRuntime({}))).toContain("describe_dataset")
  })

  /**
   * **它与 subagent 无关，不能挂在那个分支里。**
   *
   * `toolsFor` 在没有 `subagentChildEntry` 时会提前返回——
   * 数据工具挂在后面的话，那种装配里它整个消失。
   * 而「少了一个工具」不报任何错，只会让模型改用 `read` 把 CSV 塞进上下文。
   */
  it("**没配子 agent 入口时也在** —— 这是它最容易被弄丢的地方", () => {
    const 名 = 工具名(new NativeRuntime({}))
    expect(名, "没有 subagentChildEntry 的装配里数据工具丢了").toContain("describe_dataset")
    expect(名).not.toContain("subagent")
  })
