 ...
 *
 * 今天的调用点只有两处，都是 `ConnectionSurface` 上那颗**「检查配置」**
 * （`connection.tsx:52` 与 `:74`），对应 `App.tsx` 的 `exhausted` 早退分支
 * 与那条重连横幅。**两处的理由不是同一条**（2026-09-17 更正：上一版把
 * 「整屏被那层盖着」按在了重连那一支上，是假的——`connection.tsx` 的文件头
 * 自己写着 `reconnecting → 横幅（还能看已有内容）`，app 就在横幅后面照常渲染）：
 *
 * - `exhausted`：`.body` 那个网格根本没渲染，**窄栏在那里不存在**，没得选；
 * - `reconnecting`：窄栏画得出来，但 `ConnectionSurface` 只有一个
 *   `onOpenSettings`，两处配两种形状就是让同一颗按钮的结果取决于连接状态。
 *
 * 别处一律走 `开设置栏`。
 *
