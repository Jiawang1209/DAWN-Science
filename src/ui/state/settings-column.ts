 ...
 * 覆盖成 `undefined`，坞从此回不来。**只在设置从「不在场」变成「在场」的
 * 那一下才记账**，之后设置内部再怎么换分类都不该动这份记忆。
 *
 ...
 *
 * **只在设置从「不在场」变成「在场」的那一下才记账**（C1）：设置已经在场时
 * 再调一次——命令面板跳分类走的正是这条路——不该覆盖上一次记下的坞房客，
 * 否则命令面板跳两次分类，坞就永久回不来了。
 *
...
export function 开设置栏(section?: string): void {
  ...
  // 「让位」与「记账」是两件事，分开写——折成一个 if 正是 C1 的成因
  if (!$设置在场.get()) {
    setValue($被顶掉的房客, $rightDockOpen.get() ? $rightDockTenant.get() : undefined)
  }
  if ($rightDockOpen.get()) setRightDockOpen(false)
