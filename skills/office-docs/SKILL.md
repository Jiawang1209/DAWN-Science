---
name: office-docs
description: 生成、读取、编辑 Office 文档（xlsx / PDF / pptx / docx）的工具用法与纪律——大文件分页读、交付前 recalc+audit 自检、能用 Markdown 交付就不用二进制。工具在「设置 → 插件」里开关。
---


# Office 文档工具

DAWN 的 Office 插件提供 14 个文档工具（设置 → 插件 里按族开关）。移植自 dsh-office / 天枢（Apache-2.0），纪律参考 anthropics/skills。
...
- **Markdown 够用时**——更轻、可 diff、能直接在对话里读。这是最常被忽略的一条。
- **用户只是想「看看数据」**：直接读进来讲清楚，或画一张图；不必先生成一个 .xlsx 再让他打开。
- **要算东西**：算在内核里（`run_code` + pandas/openpyxl），这些工具是**排版与读写**，不是计算引擎。
  `xlsx_recalc` 是交付前的门禁，不是你做分析的地方。
- **要图**：这十四个工具**不画图**（看上面那张表：内容块里没有图表这一类）。
  图用 `run_code` 出成 png，再作为图片放进文档。
