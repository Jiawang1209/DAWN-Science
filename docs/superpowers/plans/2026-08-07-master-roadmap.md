...
> **「pi 的分层」这一条是补写的，补写本身就是教训。**
> 原先只写了「使用 pi-agent-core / pi-ai」——那不构成决策，pi 是分层库。
> 实现落成了第二层最底下的 `new Agent({tools: []})`：**agent 一个工具都没有**，
> provider 层手搓且写死 `openAICompletionsApi()`，anthropic / google 原生 API 走不通。
> 完整决策见规格旁的 `2026-08-08-dependency-layering-decision.md`。
...
| **R** | **`2026-08-08-dependency-layering-decision.md`（规格旁）** ⬜ **进行中** | **返工：坐 pi 第三层。R1 Spike A-2 → R2 native+配置 → R3 凭证 backend → R4 协议改 snapshot+revision → R5 UI 显示工具调用** | **已写（决策部分）** | 计划 2 |
| 3 | `stage2a-jupyter-kernels.md` | Jupyter 协议客户端、Ark(R)、ipykernel、中断、富输出、REPL UI | **返工 R 完成后** | 计划 2 + R |
...
**返工 R 插在 ①-B 与 ②-A 之间**的理由：native runtime 是 ②-A 的前置——内核执行也要经 agent 发起。
**带着一个没有工具的 agent 进 ②-A，等于把这个洞焊进下一层。** 返工不是可选项，是 ②-A 的准入条件。
...
### 5.1 判据的生成纪律（2026-08-08 新增，由 G1 的一次失效确立）

**G1 的三条判据全部是 PTY 的**（能敲字 / Ctrl-C 生效 / 退出后终端正常），
一条都没验 native agent。所以 `tools: []` 这个洞在 G1 通过的那一刻就已经存在，
**而门在设计时就看不见它**——作者本人认真测了三项，三项都过，问题却完好无损。

> **纪律**：决策门的判据必须**逐条对照规格的能力清单生成**，不得由实现者临时拟定；
> 且必须**覆盖每一种 runtime**（native / PTY / 将来的 ACP），
> 不能只覆盖最容易演示的那一种。
>
> 这条与 §11 已记录的另一条并列：详细计划的验收清单必须逐条对照主规划的对应决策门。
> **两条针对的是同一类失败——判据的来源不受约束。**

### 5.2 GR：返工门（2026-08-08 新增）

| 门 | 位置 | 问题 | 不通过怎么办 |
|---|---|---|---|
| **GR** | R1 结束 | **`createAgentSession()` 起得来？内置工具真能读文件、跑命令？扩展的 `tool_call` 真能 `block`？`AuthStorageBackend` 真能换成 OS 加密？** | 任一项不过 → **停在 R1，不进 R2**，并在 FINDINGS 记录失败的接口与替代路径 |

> **GR 是为上一次的失败专设的。** 上次正是拿一个只验了「pi 能跑起来」的 Spike A
> 去指导 Native Runtime 的实现——FINDINGS 里没有工具注入的签名可抄，实现就填了空数组。
> **R1 的产出物必须是 FINDINGS 里「工具注入与 provider 选择」那一节**，即当初该有而没有的那一节。
...
| **阶段 ①-B 代码** | ✅ 21 个开发任务完成，2026-08-08。438 测试通过 |
| 阶段 ①-B 验收（G2） | ⏸ **暂缓**——见下方返工说明，`tools: []` 未修复前不值得进入两周观察期 |
| **依赖分层决策** | ✅ **完成，2026-08-08**（`specs/2026-08-08-dependency-layering-decision.md`） |
| **返工 R** | ⬜ **进行中**：R1（Spike A-2 / GR 门）→ R2 → R3 → R4 → R5 |
...
**G1 判定（2026-08-08）**：三问全过，阶段 ①-A 验收通过。

- **四个会话能并存？** ✅ 四会话同时存活、pid 互不相同、输出互不串台、租约与 sessionDir 各自独立、停掉其一不影响其余（`tests/integration/concurrent-sessions.test.ts`）
- **CLI 的能真接管？** ✅ 作者本人实测确认：TUI 响应键盘（方向键/退格正常）、Ctrl-C 中断 claude 而非杀掉 CLI、退出后终端模式正确恢复
- **全局配置未被污染？** ✅ `~/.claude/settings.json` md5 全程一致

> **该门有两条记录在案的教训。**
>
> **其一**：实施计划 Task 1.11 的验收清单**漏掉了「四个会话能并存」**——它在主规划的 G1 里但没进详细计划。补测时才发现缺口。
> **推论**：详细计划的验收清单必须逐条对照主规划对应决策门，不能各写各的。
>
> **其二（2026-08-08 补记，更严重）**：**上面三条判据全部是 PTY 的**。
> 作者认真测了三项、三项全过——而 native runtime 的 `tools: []`
> （agent 一个工具都没有）在同一时刻就已经存在，**判据完全触碰不到它**。
> **推论**：判据必须覆盖每一种 runtime。见 §5.1。
>
> **G1 的判定仍然成立**（它判的是 PTY 接管，那部分是真的），但它**不构成对 native 线的任何背书**。

**G2 暂缓的理由（2026-08-08）**：作者试用后发现 native agent **一个工具都没有**（`tools: []`），
provider 层手搓且写死 `openAICompletionsApi()`，而 pi 这些全都提供。
**一个读不了文件、跑不了命令的 agent，不可能通过「你是否愿意用它替代裸终端」这一问。**
让它进入两周观察期只会得到一个必然为否的结论，且浪费两周。**先返工，再观察。**

**下一步**：**R1 —— Spike A-2，过 GR 门（见 §5.2）。**

R1 要验四件事，产出物是 FINDINGS 里「工具注入与 provider 选择」一节：

1. `createAgentSession()` 起得来
2. 内置工具（bash/read/write/edit/grep/find/ls）真能读文件、跑命令
3. 扩展的 `tool_call` 事件真能 `{ block: true, reason }` 拦下执行 ← **capability 授权门的地基**
4. `AuthStorageBackend` 真能换成 OS 加密（Electron `safeStorage`）

**R1 不过不进 R2。**

> **G2 的第二问「你是否真的开始用它替代裸终端」不由任何 Task 交付**，只能靠行为观察。
> 计划 §0 已据此把功能砍到「刚好能替代裸终端」——**但那个「刚好」算错了**：
> 它算进了对话、终端、面板，唯独漏了「agent 能干活」。返工后重估。
