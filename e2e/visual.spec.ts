...
const SCREENS: { name: string; go: (page: Page) => Promise<void> }[] = [
  ...
  },
  {
    ...
    go: async (page) => {
       ...
       * 上一版这条注释说反了）：
       * - 「插件」「MCP 服务器」这两个数**是这条分支今天才加的**
       *   （`41dbf13`，Task 1）。基线存于 `aeb7616`，`41dbf13` 不是它的祖先——
       *   所以基线里那两行没有数字是**对的**，它们那时还不存在。
       * - 「Skills」从 5 变成 6，**不是渲染变了，是内容真的多了一个**：
       *   `a74ab7a`（main，2026-09-15）加了 `skills/analyze-like-case/`。
       *   **这张基线因此整整一天都在如实反映一件已经不成立的事，而没人发现**——
       *   逐像素对比把一次真实的内容变更吞掉了。
       *
