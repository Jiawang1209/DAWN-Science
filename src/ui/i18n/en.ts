...
export const EN: Readonly<Record<string, string>> = {
  ...
  "已跑 {0}": "running {0}",
  "照这篇做": "Follow this case",
  "没有封面": "no cover",
  "这台 MCP 没有图廊": "this MCP server has no gallery",
  "封面没取到": "cover unavailable",
  "正在取封面": "fetching the cover",
  "只取本机地址上的图：{0}": "only images on a local address can be fetched: {0}",
  "取图超时：{0}": "timed out fetching the image: {0}",
  "取不到这张图：{0}——{1}": "could not fetch the image: {0} — {1}",
  "取不到这张图：{0}——HTTP {1}": "could not fetch the image: {0} — HTTP {1}",
  "那个地址回的不是图片（{0}）：{1}": "that address did not return an image ({0}): {1}",
  "图片超过 2MB：{0}": "image is larger than 2MB: {0}",
  "正在运行第 {0} 条：{1}": "running #{0}: {1}",
  "运行了 {0} 条命令": "ran {0} commands",
  "调用了 {0} 次工具": "called tools {0} times",
  "· {0} 条失败": "· {0} failed",
  "共 {0}": "{0} total",
  "{0} 字节": "{0} bytes",
  "输入 {0}": "in {0}",
  ...
  "还没有填任何 API key，暂时不能对话——点这里填一个": "No API key yet, so chatting is not possible — click here to add one",
  "首次设置": "First-time setup",
  "欢迎使用 DAWN Science": "Welcome to DAWN Science",
  "先跳过": "Skip for now",
  "模型 key": "Model key",
  "（必需）": "(required)",
  "已填 {0}": "Saved for {0}",
  ...
  "正在检测…": "Detecting…",
  "检测本机解释器": "Detect interpreters on this machine",
  "重新检测": "Detect again",
  ...
  "二进制": "binary",
  "产物": "Artifact",
  "本轮产出未知": "Outputs of this turn unknown",
  "外部 CLI 的文件操作不可见": "File operations of external CLIs are invisible",
  "远端会话或非 git 工作区，探针没跑": "Remote session or non-git workspace; the probe did not run",
  "产物清单没取到": "Could not load the artifact list",
  "打开产物 {0}": "Open artifact {0}",
  "本轮生成的文件": "Files generated in this turn",
  "GENERATED · {0}": "GENERATED · {0}",
  "另有 {0} 次运行产出未知": "{0} more run(s) with unknown outputs",
  ...
  "这段会话还没有生成文件": "No files generated in this session yet",
  "agent 新建的文件会实时出现在这里。": "Files the agent creates will show up here as they are made.",
  "（工作区根）": "(workspace root)",
  "下载 {0}": "Download {0}",
  "在文件里定位 {0}": "Locate {0} in Files",
  "面板": "Panel",
  "面板：{0}": "Panel: {0}",
  "调整面板宽度": "Resize panel",
  "关闭面板": "Close panel",
  ...
  "正在截取窗口…": "Capturing the window…",
  "这个环境里没有截屏通道，取不了色": "No capture channel here; cannot pick colors",
  ...
  "跑": "Execute",
  "运行中…": "Executing…",
  "在 {0} 内核里跑一句…（⌘↩ 运行）": "Run a line in the {0} kernel… (⌘↩ to run)",
  "会记进对话，agent 下一轮知道": "Recorded in the chat; the agent sees it next turn",
  "变量 →": "Variables →",
  "导出 .ipynb": "Export .ipynb",
  "导出 .md": "Export .md",
  "已导出 {0} 个 cell 到 {1}": "Exported {0} cells to {1}",
  "agent": "agent",
  "语言未知": "language unknown",
  "（未记录代码）": "(code not recorded)",
  "要跑的代码": "Code to run",

  /* ── 后端错误（B15，2026-09-01）──
   * 键是 `fault(code, msgid, ...args)` 的 msgid（`src/workbench/backend.ts`）与 `i18n消息()` 的第一个实参，
   * 由 `tests/workbench/fault-i18n.test.ts` 对表；「没人用的孤儿」那条也扫它们。
   * 这些话是**给人看的失败**，语气与界面其它文案一致：说清缺什么、该去哪儿改。 */
  "粘贴的这张图太大了（约 {0}MB，上限 3.5MB）": "The pasted image is too large (about {0}MB; the limit is 3.5MB)",
  "不认识这个图片格式：{0}（认得 png / jpg / gif / webp / bmp / tiff）": "Unrecognised image format: {0} (png / jpg / gif / webp / bmp / tiff are supported)",
  "读不了这张图：{0}——{1}": "Could not read the image {0} — {1}",
  "这张图太大且缩不下来：{0}（{1}MB）": "The image is too large and could not be shrunk: {0} ({1}MB)",
  "本次运行没有装配记忆": "Memory is not wired up in this run",
  "这台服务器还没连上——先连上再看它的文件": "This server is not connected yet — connect first, then browse its files",
  "读不了 {0}：{1}": "Could not read {0}: {1}",
  "{0} 是目录，不是文件": "{0} is a directory, not a file",
  "本次运行没有装配远端连接": "Remote connections are not wired up in this run",
  "本次运行没有装配任务": "Tasks are not wired up in this run",
  ...
  "本次运行没有装配项目技能目录": "The project skills folder is not wired up in this run",
  "只改技能的 SKILL.md": "Only a skill's SKILL.md can be edited",
  "自带的技能在应用包里，只读；想改就复制一份到你自己的目录": "Built-in skills live inside the app bundle and are read-only; copy one into your own folder to change it",
  "{0} 不在任何一个可改的技能目录里": "{0} is not inside any editable skills folder",
  "本次运行没有装配全局子 agent 目录": "The global subagents folder is not wired up in this run",
  "只改 .md 的定义文件": "Only .md definition files can be edited",
  "自带的子 agent 在应用包里，只读；想改就复制一份到你自己的目录": "Built-in subagents live inside the app bundle and are read-only; copy one into your own folder to change it",
  "{0} 不在任何一个可改的子 agent 目录里": "{0} is not inside any editable subagents folder",
  "刚打开的项目取不到摘要：{0}": "Could not fetch a summary for the project just opened: {0}",
  "Run \"{0}\" 不存在": "Run \"{0}\" does not exist",
  "资源 \"{0}\" 没有溯源记录": "Resource \"{0}\" has no provenance record",
  "配置里没有叫「{0}」的 agent": "There is no agent named “{0}” in the config",
  "「{0}」的手到不了服务器——它自己读写文件、跑命令，都在本机。远端会话请用 API 模型，或标了「能上服务器」的 ACP 适配器（如 claude-code-acp）。": "“{0}” cannot reach the server — it reads files and runs commands on this machine only. For a remote chat use an API model, or an ACP adapter marked “works on servers” (such as claude-code-acp).",
  "没有这个任务：{0}": "No such task: {0}",
  "名单里没有这台：{0}": "That server is not on the list: {0}",
  "本次运行没有装配配置文件，加不了": "No config file is wired up in this run, so nothing can be added",
  "这段 JSON 里没有服务器的名字（只有 command），请另外给它起一个": "This JSON has no server name (only a command) — give it one",
  "本次运行没有装配配置文件，删不了": "No config file is wired up in this run, so nothing can be removed",
  "本次运行没有装配配置文件，改不了": "No config file is wired up in this run, so nothing can be changed",
  "本次运行没有设置存储": "Settings storage is not wired up in this run",
  "没有叫「{0}」的插件。有的是：{1}": "There is no plugin named “{0}”. Available: {1}",
  "本次运行没有装配设置": "Settings are not wired up in this run",
  "「{0}」插件没有「{1}」这一族。有的是：{2}": "The “{0}” plugin has no “{1}” family. Available: {2}",
  "{0} 没有完整的 frontmatter（开头结尾各一行 ---），不敢改": "{0} has no complete frontmatter (a --- line at the start and the end), so it was left alone",
  "本次运行没有装配废纸篓": "The trash is not wired up in this run",
  ...
  "本次运行没有配置文件，视觉服务没地方保存": "There is no config file in this run, so the vision service has nowhere to be saved",
  "没有这条定时任务：{0}": "No such scheduled task: {0}",
  "本次运行没有装配调度器": "The scheduler is not wired up in this run",
  "这段对话还没有 cell，没有东西可导": "This chat has no cells yet — there is nothing to export",
  "这次运行没有 native 运行时，做不了提示词增强": "This run has no native runtime, so prompt enhancement is unavailable",
  "「{0}」还没有可用的 API key——到设置里填一个（应用更新后，上一版存的 key 可能解不开，要重新填）": "“{0}” has no usable API key — add one in Settings (after an app update, a key saved by the previous version may no longer decrypt and has to be re-entered)",
  "增强超时了，这次没改": "Enhancement timed out; nothing was changed",
  "要不到二维码：{0}": "Could not get a QR code: {0}",
  ...
  "{0} 已就绪": "{0} is ready",
  "{0}% · {1} / {2}": "{0}% · {1} / {2}",
  "在后台下，你接着干活": "It downloads in the background; carry on working",
  ...
  "查看发布说明": "Read the release notes",
  "这一版不再提醒": "Don't remind me about this one",
  "关于": "About",
