...
test("**填完就能用**：自定义端点 → 模型选择器 → 真的连上了", async ({ dawn }) => {
   ...
   *
   * 两颗并成一颗之后，「填完就能选到它」在模型清单里看——
   * 新配好的那一家作为一个**分组**出现在里面。
   * 这条用例的验收没变：**填完表它自己就在了，我们从没点过「建 agent」**。
   */
  await page.locator(".model-pill .model-trigger").click()
  // 新那一家自己成一组，点它的模型就换过去了
  await page
    .getByRole("menu", { name: "切换模型" })
    .getByRole("menuitem", { name: /my-7b/ })
    .click()
