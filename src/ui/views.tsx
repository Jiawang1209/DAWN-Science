...
export function DockSwitch({
   ...
   *
   * ## 名字两态（2026-08-20 作者定的，替掉「一直是面板：<谁>」那一版）
   *
   * 作者：*「面板默认情况下就叫面板，我打开之后可以变为审阅、文件、概览、
   * 网页，关闭了以及初始化 App 的时候就叫做面板。」*
   *
   * - **关着 → 「面板」**：上一次开的是谁是实现的记忆，不该在关着时漏出来；
   * - **开着 → 「面板：<谁>」**：报的是此刻真开着的那一格，跟着标签条走。
   *
   * 与「取消⇄确认」那类两态按钮同一口径：名字说的是**此刻的事实**。
   * 开没开另有 `aria-expanded` 报给读屏。
   *
...
export function SessionSidebar({
  ...
  /**
   * **人刚刚亲手选的那个工作目录**（2026-08-19）。给了就让它出现在「项目」那一列，
   * 哪怕它还一个任务都没有。
   *
   * **不是「当前活跃项目」**：应用一启动就有一个活跃项目，
   * 拿它当判据会让这一列永远存在（第一版就是这样，十七条 e2e 当场红）。
   */
  刚选的工作区?: string | undefined
}) {
   ...
   *
   * ## 判据是「他刚选了它」，不是「有一个活跃项目」
   *
   * 我第一版写的是后者——**十七条 e2e 当场红，含全部十张视觉基线**。
   * 原因很简单：应用一启动就有一个活跃项目（默认工作区），
   * 于是那一列**变成了永远存在**，连一个会话都没有的新窗口上也挂着它。
   *
   * 那不是他要的。他要的是**「我刚选的这个」看得见**，
   * 而「刚选」这件事只有 `App` 知道（那颗按钮按下去的那一刻）。
   *
   * ## 为什么不干脆把所有项目都列出来
   *
   * 列全部的话，**每一个曾经点开过又没用的文件夹都会永远留在那儿**，
   * 而这一列是「我在哪几摊活儿之间来回」，不是「这台机器上有哪些文件夹」。
   * 只补刚选的那个：再选一个，上一个自己就退场（它本来也没有任何东西）。
   *
   * **它排在最前**：刚选的那个正是此刻要看的。
   */
   ...
   * 「梳理数据中心介绍思路」。
   *
   * 三个收纳给的东西不一样，这是作者逐条定的（2026-08-15）：
   *
   * | 收纳 | 给什么 | 为什么 |
   * |---|---|---|
   * | 项目 | **完整路径** | *「不能仅仅带有文件夹的名字，还要有路径」*——
   * |      |            | 同名文件夹在不同盘符下是常事，光有名字答不了「哪一个」 |
   * | 服务器 | 机器名 / IP | 作者：*「直接显示 IP 地址很好」* |
   * | 会话（临时）| **不给** | 作者：*「不用文件夹位置了，因为没必要」*——
   * |          |          | 那个目录是服务端给的，人从没选过它 |
   *
   * 最后一条印证了 `createTask` 里早就写下的那句：*「它不进 `TaskSummary`：
   * 摆出来只会让人看见一个自己从没选过的路径」*。**那条判断一直是对的。**
   */
  const 属于哪儿 = (task: TaskSummary): string | undefined =>
    task.connectionId
      ? (服务器名?.(task.connectionId) ?? task.connectionId)
      : // **完整路径，不是基名**：同名文件夹在不同位置是常事
        task.workspace
