...
import { 改造成远端工具, 挑工具后端, 解析远端路径 } from "../../src/remote/tools.js"
...
describe("**只换 execute，别的一字不改**", () => {
  ...
  it("**不认识的工具原样留下** —— 悄悄丢掉比留着更错", () => {
    ...
    const 出 = 改造成远端工具(定义, { executor: ex, workspace: WS })
...
describe("路径守卫", () => {
  ...
  it("**`..` 先归一再判** —— 只查前缀的话，`../../etc/passwd` 会大摇大摆通过", () => {
    expect(() => 解析远端路径(WS, "../../etc/passwd")).toThrow(/越出/)
    expect(() => 解析远端路径(WS, `${WS}/a/../../../etc`)).toThrow(/越出/)
  })
...
describe("挑执行后端", () => {
  it("没给远端 = 原样用本地那份", () => {
    const 原 = 原定义()
    expect(挑工具后端(原, "/w", undefined)).toBe(原)
  })

  it("给了远端 = 四个工具全部换成远端版", async () => {
    const { ex, 跑过 } = 假执行器()
    const 出 = 挑工具后端(原定义(), WS, ex)
