...
test.describe("正在跑", () => {
  ...
  /**
   * **切走之后那个标记还得准**——这条是探针拍下来的一个洞（2026-08-19）。
   *
   * ## 洞长什么样
   *
   * 「跑着」的数据来自推送流，而 `events.bump` 里有一句
   * `if (!this.subscribed.has(sessionId)) return`；渲染进程**切会话时会退订**。
   * 于是：切走一段正在跑的对话，它的「跑着」**永远挂在那儿**，活干完了也不掉。
   * 量到的（`sleep 6` 的活，切走之后盯 11 秒）：`跑着 / 跑着 / 跑着 …`，一次没变。
   *
   * ## 为什么它不是边角
   *
   * **「你切走了也想知道它在跑」正是作者选这个形状的理由**（三选一那一问里
   * 明写着）。也就是说，这颗标记唯一真正有用的场景，恰恰是它说谎的场景。
   * 而它说的是**会粘住的谎**：不像闪一下的错，它就那么亮着，
   * 直到你重启应用。
   *
   * 修法是「它还在替我干活，我就继续听着」：跑着时不退订，跑完再补退订。
   * **这条用例同时盯两头**——标记要变回去，而那份订阅也要还回去。
   */
  test("**切到别的会话，跑完了那一格也要变回去**", async ({ dawn }) => {
    const { page } = dawn
    /**
     * 两段都**不让夹具说话**：假模型默认只在第一轮调工具（`once ?? true`），
     * 让夹具说一句就等于把那一次用掉了——而要拖住的恰恰是这条用例自己发的那轮。
     */
    await 开一段临时会话(page) // 乙：待会儿切过去
    await 开一段临时会话(page) // 甲：正在停留的这一段

    const 框 = page.getByPlaceholder(/今天帮你做些什么/)
    await 框.fill("跑一个长的")
    await 框.press("Enter")

    const 甲 = 那一格(page, "跑一个长的")
    await expect(甲).toHaveText("跑着", { timeout: 30_000 })

    // **点侧栏切到乙**（不是重载——重载会把渲染进程的状态整个冲掉，
    // 那样这个洞根本暴露不出来。第一版探针就是这么误判成「没问题」的）
    await page
      .locator(".session-list .sess-item")
      .filter({ has: page.locator(".sess .name", { hasText: "新会话" }) })
      .locator(".row")
      .first()
      .click()

    // 人已经不在甲上了，但甲跑完之后那一格必须变回时间
    await expect(甲, "切走之后那个「跑着」再也不掉了").not.toHaveText("跑着", {
      timeout: 60_000,
    })
  })
})
