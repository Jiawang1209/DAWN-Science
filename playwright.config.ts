...
export default defineConfig({
  ...
  /**
   * 单条用例的预算。**2026-08-10 从 60s 提到 150s。**
   *
   * ②-A 之后，一条 e2e 里可能同时起 **Electron + 一个真 Jupyter 内核**，
   * 而机器忙的时候光是「Electron 开出第一个窗口」就要几十秒
   * （实测过 `firstWindow: Timeout 30000ms exceeded`）。
   *
   * 今晚全量里红过三次，每次是**不同**的用例，单独跑都过——
   * **根因是同一个阈值，不是三个 bug**。
   *
   * **放宽它不掩盖任何东西**：真坏了的话等再久也不会好，测试照样红。
   * 掩盖的只是「这台机器此刻很忙」，而**因为机器忙而红的测试会教人忽略红色**。
   */
  timeout: 150_000,
  /**
