...
function addSlide(
  ...
  const font = theme.fontFace !== undefined ? { fontFace: theme.fontFace } : {}
  ...
  },
  {
    ...
    execute: async (args) => {
      ...
      try {
        ...
        const include = new Set((typeof args.include === 'string' ? args.include : '').split(',').map((s: string) => s.trim()).filter(Boolean))
