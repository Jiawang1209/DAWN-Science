...
| **阶段 ①-B 代码** | ✅ **13 个开发任务完成，2026-08-08。327 测试通过** |
| 阶段 ①-B 验收（G2） | ⬜ **待作者**：Task 2.14 功能验收（需交互）+ Task 2.15 两周行为观察期 |
...
> **该门有一条记录在案的教训**：实施计划 Task 1.11 的验收清单**漏掉了「四个会话能并存」**——它在主规划的 G1 里但没进详细计划。补测时才发现缺口。
> **推论**：详细计划的验收清单必须逐条对照主规划对应决策门，不能各写各的。后续阶段的计划编写须加此检查。

**下一步**：作者本人完成 Task 2.14 功能验收与 Task 2.15 的两周行为观察。

> **G2 的第二问「你是否真的开始用它替代裸终端」不由任何 Task 交付**，只能靠行为观察。
> 计划 §0 已据此把功能砍到「刚好能替代裸终端」，以尽早进入观察期。
