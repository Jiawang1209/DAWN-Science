...
### 方法换了：**整屏扫，不再一处处问**（2026-08-12）

作者：*「我觉得你1:1的给我复刻 workbuddy 的UI 其实不是什么难事，
但是为什么我感觉你好像没有竭尽全力的感觉。」*

他是对的，而且证据就在上面这几张表里：**它们是一条条被指出来之后才量的**。
工作目录那个入口是最直接的例子——我从没量过它在哪，自己想了个位置放上去，
放在了对话标题栏，而它在**输入卡下面那一行**。

所以方法改了：`scratchpad/wb-sweep.mjs` **一次把整屏的可见元素扫成一棵树**
（几何 + 字号 + 颜色 + 圆角 + 内距 + 阴影），我们自己那边用同一份逻辑扫一遍，
两棵树摆在一起逐条对。**一次扫出来的差异比之前所有轮加起来还多。**

### 侧栏（2026-08-12 整屏扫出来的）

| 项 | WorkBuddy | 我们（改后） |
|---|---|---|
| 侧栏宽 | `264` | ✅ 264 |
| 行 | `240×31` @x=12 · 圆角 `8` · `padding: 5px 12px` · `gap: 8` | ✅ 全同 |
| 行内字 | **`14px / 21px`** | ✅（原先 13px——**外壳 13 这条我理解过头了**，它只有分区标题与副标签才降到 12/11） |
| 图标 | `16×16`，色 `.7`——**比文字（`.9`）淡一档** | ✅ |
| 分区标题 | `240×30` · `padding: 5px 12px` · `12/20/600` · `.5` | ✅ |
| 会话行 | **`240×31` 单行**：标题在左、时刻在右 | ✅（原先双行 `53`，**高出七成**） |
| 分割线 | **一条都没有** | ✅ 去掉了底部三条 `border-top` |
| Logo | `12px/20px/700` 色 `.5`，版本号 `10px` | 待办 |
| 底部用户行 | `164×44` 圆角 8 + 两颗 `32×32` 圆角 6 | 我们没有这块 |
| 侧栏底色 | `rgb(242,242,242)` | `rgb(243,243,243)`（灰阶差一档，暂不动种子） |

### 输入卡下面那一行（`wb-input-footer`）

| 项 | 实测 | 我们 |
|---|---|---|
| 那一行 | 与输入卡同宽 · 高 `32` · `padding: 0 12px` · `gap: 4` | ✅ |
| chip | `138×32` · `padding: 0 8px` · 圆角 `8` · **无边框无底色** · `gap: 4` | ✅ |
| chip 文字 | `14px/20px` · 色 `rgba(0,0,0,.5)` —— **比正文弱一档** | ✅ |
| 里面有什么 | `📁 选择工作空间 ⌄`、`✓ 默认权限 ⌄` | 工作目录已接；权限我们还没有这个概念 |

**还没量的**：侧栏 hover / 选中态的具体色值、暗色映射、
输入卡上面那一行（`+`、模型 pill、麦克风、圆形发送键的精确几何）、
以及它那个下拉浮层里到底列了什么。**应用开着调试端口时随时可以再量。**
