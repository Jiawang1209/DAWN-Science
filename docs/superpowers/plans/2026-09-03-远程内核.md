...
export async function 隧道(c: 可转发, 远端端口: number): Promise<一条隧道> {
  ...
  const server = createServer((sock) => {
    ...
    c.forwardOut(远端端口).then(
      ...
      // 远端拒了（sshd 关了 AllowTcpForwarding 之类）：RST 掉本地这条，客户端立刻看到 error 而不是等一个干净的 FIN
...
（第一条的期望值是**改动之前**在 main `5f909a5` 的代码上算出来的（2026-09-04 主线程算的），不要改它——这条就是「老行不裂」的证据。）
