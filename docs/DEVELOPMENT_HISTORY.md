...
### 2026-09-18 — 输入框：文字的真身放回 DOM，组词不再被重渲染抹掉

- **Type**: fix
- **Motivation**: 作者打不出中文：*「我打『这样』`zheyang` 的时候只能打出 `zh`，
  然后就自动退出中文了。`zhe shu` 这种带有 `zh` 的，其实汉语拼音都有问题。」*
  这条账从 2026-09-08 起就挂着（`ime-trace.ts` 的文件头里逐字记着同一句），
  一直**复现不出来**。这一轮把它定了性。
- **What**:
  - **先把范围收死，再动代码**。作者自己做的两组对照决定了一切：
    ① 终端（Codex/Claude）正常——但终端走 macOS 原生 Cocoa 的输入通路，
    与 Chromium 不是同一套实现，所以这条只能说明「问题在 Chromium 层或我们这层」；
    ② **Chrome 正常**——Chrome 与 DAWN 用的是同一套 Chromium，**它好我们坏，账就在我们这一层**。
    作者当场否掉了「你用的哪个输入法」这个问题（*「其实没有关系」*）：同机同输入法、
    别处都好，输入法身份对定位不提供信息。他是对的。
  - **机制是量出来的**。台架照搬输入框的接线（受控 `value={draft}`，每个字母绕一圈全局 store
    再回到 DOM），两条路各跑一遍：组词期间 `input` 照常派发时 state 跟得上、无关重渲染写回
    同一个字符串，没事；**`input` 没到时 state 停在旧值，一次与打字无关的重渲染就把框里的
    组词文本抹成空**。相对时间跳一秒、会话列表刷新、状态栏变化，都算。它专挑 `zh` 这类
    两个字母的组合，只因为**组词窗口越长，撞上的概率越大**。
  - **所以不去堵某一次重渲染**——那是把一整类问题当成一条路径修。Hermes 与 Codex 的输入框
    遵守同一条规矩：**组词的时候 DOM 说了算，React 不往回写**（前者 `contentEditable`，
    组词期间跳过 `input`、`compositionend` 冲一次；后者 ProseMirror 的 `domObserver`
    在 `composing` 时推迟同步）。新组件 `src/ui/composer-field.tsx` 是同一条路的轻量版：
    textarea 改成**非受控**，只有「外面真的换了内容」才写 DOM（换会话、插入 `@` 引用、
    发送后清空），**组词期间一个字都不写**，`compositionend` 之后以人手上那句为准
    （有些输入法组完词不再补 `input`，不主动冲一次的话外面不知道框里有字、发送键不亮——
    Hermes 为这一条留过一个复现测试）。
    保住 textarea 是刻意的：粘贴、`@` 菜单定位、镜像高亮层、几十条 e2e 都站在它上面；
    换 `contentEditable` 的好处（能砍掉镜像层）眼下用不上，不值那个风险面。
  - **三处都换了**：对话里的输入卡、空态那张输入卡、「修改我说过的一段话」那个框。
  - **第二轮：完全对齐 Hermes**（作者定的：*「完全按照他们的做法去做」*）。第一版还保留着
    「组词期间照常往外同步」——方向是安全的（危险的一直是反方向），但把半截拼音当成正在输入的内容
    有副作用：`/` 与 `@` 菜单可能被 `zhe` 这种半成品触发、发送键提前亮。**他们三家都不这么干。**
    现在是 Hermes 那一句 `onInput(e) { if (composing) return }`：组词期间一个字都不报，
    `compositionend` 一次性交出终值——**那一冲因此成了这条路唯一的出口**，不冲外面就永远不知道框里有字。
  - **由它带出来的一处必改**：`onSubmit` 原先读的是渲染闭包里的 `draft`。组词期间不再同步之后，
    「组完词紧接着回车」那一瞬草稿有可能还停在组词之前——**发出去的会是上一句，或者干脆是空的**。
    两处发送都改成从 DOM 读（`输入框.current?.value ?? 草稿`）：既然文字的真身归 DOM，
    发送这一刻就该问 DOM。**这是「组词期间不同步」的代价，不是一处顺手的清理**——
    不一起改就是拿一个静默的错发换一个菜单误触。
  - **`.control` 那条契约比原先更硬了**。`design-contract` 的扫描读源码文本，
    看不见「类名从 props 传进来」，把新组件判红。**没有给它开口子**——改成组件自己把
    `control` 并进类名，再加一条断言直接量渲染出来的 `classList`：这把尺子连
    「调用点忘了写」都挡得住，而那正是 2026-08-09 那张截图里发生过的事，旧扫描挡不住。
    扫描只对这一个文件放行，理由写在旁边。
- **Impact**: 中文（以及任何要组词的输入法）在三个输入框里都不再被重渲染打断。
  输入框的值从此只走 `值` / `on值变` 这一对——`onChange` 与 `value` 在类型上就被挡在外面，
  **想把它改回受控得先绕过类型**。
- **Verification**: 判据**取过基线**——把组件换回 `value={值}`，五条里当场红两条
  （正是那条 bug 的形状），所以它不是空转。typecheck 干净；单元 **2930 passed**（236 个文件）；
  对齐 Hermes 那一轮之后重跑：单元 **2931 passed**、整套 e2e **498 passed**，
  只剩 `remote-kernel` 那张图红（本机 matplotlib 环境）。
...
### 2026-09-18 — 设置右栏 Task 7–9 · 先量再改：十四块里只有六块真装不下

- **Type**: feat
- **Motivation**: Task 6 把那一栏接上之后，剩下的问题是「那十四块内容页原本是照整页宽度做的，
  塞进 380px 会怎样」。规格写的是**先量，再改**——不预先猜哪几块要修，让判据去点名。
- **What**:
  - **Task 7 · 一条会点名的判据**（`e2e/settings-column.spec.ts`）：逐块进去，在
    `RIGHT_DOCK_DEFAULT`（380px）的栏里量 `.dock-body` 的 `scrollWidth - clientWidth`。
    比计划原文多了两把尺子，各自对着一种**假绿**：
    - **第二把尺子走遍所有后代**，专挑 `overflow-x` 是 `auto`/`scroll` 且真滚起来的那些。
      一个子元素自己带横向滚动条时，它把溢出吃进肚子里，外面那只盒子照样是 0——
      而「拿 `overflow-x: auto` 把装不下改写成你自己去找」正是计划明令禁止的
      （与规格 7.5「失败必须出声」正面冲突）。**这把尺子让那条捷径在结构上走不通。**
      `overflow: hidden` + 省略号不算：省略号在屏幕上说了「这儿被截了」，它是出声的。
    - **每进一块先等 DOM 静下来**（后代数 + 内容高连着两拍不变）再量，并把
      「这一块画出了几个元素」一并量回来、由单独一条断言点名空盒子。好几块要等 IPC 才画，
      没画完就量，盒子当然不溢出。
  - **量出来六块真挤，另外八块是 0，所以这轮 CSS 里也没有它们的份**（全部只在
    `.settings-column` 作用域内，整页一个像素不动）：长路径 `<code>` 撑破三块
    （文件引用 +7px、MCP +29px、子 agent +107px，用 `overflow-wrap: anywhere` 而不是
    `break-word`——只有前者把断点算进 min-content 宽度，也就是只有它能让外面的容器真的缩下来）；
    一行设置在窄栏里改成上下排（内核 +69px、工作目录 +320px，根因是 `.set-control .control.path`
    写死 `22rem`；`min-width: 0` 是必需的，不清掉网格项默认的 `min-width: auto`，
    `minmax(0, 1fr)` 也拦不住它——工作目录那条路径的省略号正是被这一条卡住才没生效）；
    筛选键与标签折行（子 agent 的六颗分组键横着排出去 74px，MCP 那块 29px，
    **它们被 `.skills-page` 自己的滚动条吃掉了**，屏幕上一声不吭）；用量那张饼竖过来（+52px）。
  - **顺手修掉一条会飘的老判据**（不是这一轮弄坏的）：「十四行的 `›` 落在同一条右边缘」
    此前在那一栏的 `grid-template-columns` 过渡途中就开量，报出
    `箭头右缘不齐：1266 / 1285 / 1293`——**三个数全都越过了 1280 的视口右缘**，
    只有行被撑出去才会这样。取基线证实过：不改的版本五遍里红一遍，改后 45/45 全绿。
    等待提成 `等这一栏滑到位`，两条判据共用一份。
  - **Task 8 · 视觉基线多一屏**（设置栏-亮色 / 设置栏-暗色）：上面那张「设置」截的是展开后的整页，
    这张截的是点「设置」真正看到的窄栏。**没有用 `test:e2e:visual:update`**——它带 `=all`，
    会把现存 10 张一并重写，而 Task 7 的 CSS 只作用在 `.settings-column` 里，那 10 张当时是绿的、
    没有理由动。直接跑一遍让 Playwright 把缺的两张写出来，旧的一张没碰。分类列那四个计数
    提成一份共用的 `分类计数`，整页和窄栏共用——两处各抄一张表，迟早有一处绿着说谎。
  - **Task 9 · 自己看一遍**：计划写的是 `npm run app`，实际改成隐藏窗口 + 逐步截图的一次性探针
    （用完即删）。理由是本项目踩过的那条「弹出的窗口抢焦点，作者敲的字进了被测输入框」，
    而当时作者正在排查输入法的事。四条动线逐张看过：开栏时对话还在、下钻/返回、
    展开/收回、面板让位之后自己回来。
- **Impact**: 那一栏里十四块都装得下，且**不是靠内层滚动条装下的**。整页的样子没有变化
  （所有规则都带 `.settings-column` 前缀）。视觉基线从 10 张变 12 张。
- **Verification**: `typecheck` 干净；单元 **2924 passed / 10 skipped**；视觉基线存完连验两遍
  都是 `12 passed`；整套 e2e **494 passed**（Task 8 那两张之前是 492），两条红都查明且与本轮无关——
  `remote-kernel` 那张图是本机 matplotlib 环境（计划里写明预期），
  `web-link` 是用例把本机服务写死在 58232、而 macOS 临时端口范围从 49152 起，
  这个号正被一条长期存活的 `xray` 连接占着（`lsof` 查过同一个 socket 四十分钟没变）。
  后者是一条真缺陷，但属于「网页预览」那一块，留给作者定。
...
- **What**:
  ...
  - **顺带补上「设置」那一屏缺的等待**：`visual.spec.ts` 此前不等分类列上那四个
    计数（Skills / 子 Agent / 插件 / MCP 各走一发 IPC）。这一条**不是「一条一直都在的
    竞态」**（那是我第一版写的，2026-09-17 审查查实后更正，来历各不相同）：
    - 「插件」「MCP 服务器」这两个数**是这条分支 2026-09-16 才加的**（`41dbf13`，Task 1）。
      基线存于 `aeb7616`（2026-09-11），而 `git merge-base --is-ancestor 41dbf13 aeb7616`
      为假——基线里那两行没有数字是**对的**，它们那时还不存在。
    - 「Skills」5 → 6 **不是渲染变了，是内容真的多了一个**：`a74ab7a`（main，
      作者日 2026-09-14 / 提交日 09-15）加了 `skills/analyze-like-case/`。
      **从它落地到 09-16 被发现为止，这张基线一直在掩盖一次真实的内容变更**——
      逐像素对比把它吞掉了，没有任何人收到消息。
    所以等待写成了**断值**（`toHaveText("6"/"22"/"3"/"0")`）而不是「有没有」：
    截图在构造上不可能落在过渡态，而下一次有人加一个技能，报出来的是
    `expected "6", got "7"` 而不是一张没人看得懂的像素 diff。
- **Impact**: 点「设置」不再顶掉对话。右边那一列同时只有一位房客（窄栏 or 坞），
  互斥仍然只住在 `state/settings-column.ts` 一个文件里。`打开设置整页` 换了家，
  从 `state/index.ts` 导出的名字不变。**两张视觉基线（设置-亮色 / 设置-暗色）已重存**：
  整页多了一颗「收回」、标题那一行随之高了 4px，外加上面说的那两类计数变化——
  先逐一看过 diff 图、把四个计数改成断值之后才重存，`-g "设置" --update-snapshots=all`
  只动这两张，随后连验两遍都是 `10 passed`。
- **Verification**: 八条新 e2e 先跑红（`8 failed`，全是 `.settings-column` 找不到）再跑绿
  （`8 passed`）；`npm run typecheck` 干净；`npm test` 235 文件 / 2924 条全绿；
  `e2e/*.spec.ts` 每个文件都在最终构建上跑过，唯一的红是 `remote-kernel.spec.ts:357`——
  它在本分支起点 `6d3f1d2` 上重建后单跑同样失败，与本轮无关
  （`Test timeout 60s` 撞上一条 `toBeVisible({ timeout: 90_000 })`）。
...
- **Type**: fix
- **Motivation**: 代码评审在 Round 3 结束时点出，贯穿 C1、R2-1 的其实是同一条更底层的规则——
  `$settingsColumnOpen && $rightDockOpen` 不该同时为真——而 `开设置栏` 从一开始就顶坞，
  `展开设置`、`收起设置` 却从没顶过。① `展开设置`：从「窄栏与坞万一同时开着」（不是正常路径，
  但没有代码拦着它出现）展开时，坞会跟着活下来，文件头「展开到整页时那一列空着（作者选的）」
  那句话在这条路径下是假的——原来「空着」全靠 `开设置栏` 提前顶过，`展开设置` 自己从不检查。
  ② `收起设置`：**这一个是真实可达的**——从整页 + 坞开着（`坞上位()` 造出的「一边看设置一边看图」，
  作者保留的合法状态）按 ⤡ 收起，会得到窄栏开着**并且**坞开着，坞看不见但仍是开的，侧栏再点一次
  那个房客会撞上 `点开房客`「已经是它 → 收起」那一支，关掉一个用户看不见的坞——2026-08-10/08-11
  「没有这个功能」的同一种症状：状态变了，画面没变。评审还指出，逐场景测这类漏洞挡不住下一个
  同类缺口，要求补一条覆盖「五个起点 × 六个动作」的不变式用例。
- **What**: `src/ui/state/settings-column.ts`——抽出私有帮手 `顶掉坞如果开着()`（坞开着就记账、
  关坞，返回是否真的顶掉了一位），`开设置栏`、`展开设置`、`收起设置` 三处共用它，不再各写各的。
  `展开设置` 与 `收起设置` 的文档补充「这不是禁止坞出现在整页旁边——展开/收起之后坞可以重新开起来
  与整页并排，『空着』只管这一下」，避免下一个读者把「空着」误解成「整页永远不许有坞」而把
  `坞上位()` 的「一边看设置一边看图」修没了。文件头新增「不变式（Round 4）」小节，把
  `$settingsColumnOpen && $rightDockOpen` 不能同时为真写成一条独立的规则，而不是散在三个函数各自
  的注释里。`tests/ui/settings-column.test.ts` 新增：`展开设置` 的漏顶场景、R3-2（`resetAllState()`
  不清 `$被顶掉的房客`，脏记忆渗进下一次打开）、以及一条表驱动的不变式用例（五个起点 × 六个动作，
  断言 30 次「窄栏与坞不同时开着」）。
- **Impact**: 行为修正，仍未接入 `App.tsx`。`顶掉坞如果开着` 是模块内部帮手，不导出，不影响
  `state/index.ts` 的转出列表。
- **Verification**: 新用例先红：`npx vitest run tests/ui/settings-column.test.ts` → `2 failed | 16
  passed (18)`，失败的正是 `展开设置` 漏顶场景、以及不变式用例里「只有坞开着 → 收起设置()」那一格
  ——与评审描述的两个洞完全对应。改完后同一条命令 → `18 passed`。另外临时删掉 `开设置栏` 里
  「记 `undefined`」那一支，验证 R3-2 用例确实会红（`1 failed | 17 passed`），确认不是摆设用例，
  再改回来。`npm run typecheck` 无输出。`npm test` → `235 files passed / 2921 passed | 10 skipped`
  （较 Round 3 多 3 条，无回归）。

### 2026-09-16 — Round 2 的判据本身留了一个缺口：整页设置 + 坞都开着时跳分类，坞照样回不来（设置右栏 Task 3 · Round 3）

- **Type**: fix
- **Motivation**: 代码评审用真实用例测出 Round 2 那次修复本身不完整（Important，非 Critical）：
  `开设置栏` 把「该不该记账」的判据写成 `!$设置在场.get()`（设置在不在场），这个判据在窄栏那条路上
  凑巧算对，但在整页那条路上会漏账——整页设置开着、坞也开着时再跳一次分类，`$设置在场` 已经因为
  整页而是 `true`，记账那一步被跳过，可坞照样在下一行被 `setRightDockOpen(false)` 顶掉：顶掉了却
  没有留下记录，`关掉设置` 因此翻不出上一位，坞永久回不来。这条路径就是 Task 6 计划里明文要接的：
  设置 → 展开整页 → 点「面板」（`坞上位()` 不碰 `$view`，于是整页设置与坞同屏）→ 命令面板跳到
  「MCP 服务器」→ 又调一次 `开设置栏("mcp")`。同一轮评审还指出 `开设置栏` 上方那句「让位与记账是
  两件事，分开写——折成一个 if 正是 C1 的成因」把因果说反了：C1 的成因是「每次调用都无条件重记」，
  与是否共用一个 `if` 无关；本次的修法恰恰是把两件事重新合进同一段判断里（判据从「设置在不在场」
  换成「坞是不是真的被顶掉」），因为「顶掉」与「记账」这次就是同一件事的两面。
- **What**: `src/ui/state/settings-column.ts`——`开设置栏` 的判据改写成：坞开着就无条件记下当前
  房客并关坞（不再看设置在不在场）；坞本来就关着时才去看「设置是不是从无到有」，是的话记
  `undefined`，已经在场则说明上一次已经记过账、不覆盖。文件头新增「Round 3」小节说明这条判据的
  由来；函数文档重写，把「判据问的是『这一下有没有真的顶掉一位』」讲清楚，两个分支各自的理由都
  写明，不再用已经被推翻的「只在不在场→在场那一下记账」措辞。`tests/ui/settings-column.test.ts`
  新增一条钉死这个场景的用例（`$view = "settings"` + 坞开着 → 跳分类 → 关掉设置，坞必须回来）。
  同时改正 Round 2 那条日志（上一条）里已经过时的说法：「只在设置从不在场变成在场的那一下才记账」
  不再是最终判据，标注为已被本条替换。
- **Impact**: 行为修正，仍未接入 `App.tsx`。这一轮改的只是 `开设置栏` 内部判据，导出符号、
  函数签名都没变，不影响任何调用方（目前也确实没有调用方）。
- **Verification**: 新用例先红：`npx vitest run tests/ui/settings-column.test.ts` → `1 failed | 14
  passed (15)`，失败的正是新加的整页+坞同开跳分类那条，报的错误与评审描述的现象一致
  （`$rightDockOpen` 关掉之后没能在 `关掉设置` 时恢复）。改完判据后同一条命令 → `15 passed`。
  `npm run typecheck` 无输出。`npm test` → `235 files passed / 2918 passed | 10 skipped`（较 Round 2
  多 1 条，无回归）。

### 2026-09-16 — 修两个 Critical：命令面板跳分类会冲掉坞的记忆、整页与窄栏能同屏（设置右栏 Task 3 · Round 2）

- **Type**: fix
- **Motivation**: 代码评审拿真实用例跑出两个 Critical：① `开设置栏` 在**每一次**调用时都重记
  「被顶掉的是谁」，而命令面板跳分类（Task 6 会接的 `settings.*` 五条命令）走的正是再调一次
  `开设置栏(section)`——第二次调用把第一次记下的坞房客覆盖成 `undefined`，坞从此回不来（C1）；
  ② `开设置栏` 完全不管 `$view`，从整页直接开窄栏会让整页设置与窄栏设置同屏，而不是整页让位（C2）。
  另外 mutation 测试发现两条用例是「假绿」：删掉 `关掉设置` 里的 `setRightDockTenant(上一位)`、
  或删掉 `开设置栏` 里记 `undefined` 的分支，7 条用例照样全绿——因为 `beforeEach` 没重置
  `$被顶掉的房客`，用例之间在互相借上一条留下的状态。
- **What**: `src/ui/state/settings-column.ts`——`开设置栏` 改成**只在设置从「不在场」变成「在场」的
  那一下才记账**（判据是 `!$设置在场.get()`），已经在场时再调一次只换分类、不碰记忆；同时无条件把
  `$view === "settings"` 收回 `conversation`，让窄栏与整页不再同屏。**这个判据本身留了一个缺口——
  整页设置 + 坞都开着时跳分类会漏账，被 Round 3（上一条）修正为「坞是不是真的被顶掉」，不再是
  「设置在不在场」，读这里请以 Round 3 为准。** 新增 `坞上位()`——`right-dock.ts`
  那批开坞的路径（`点开房客` 等）不知道设置这回事，也不能反过来 import 这个模块（会成环），
  所以反方向的互斥只能住在这里；它清空 `$被顶掉的房客`，因为人这次是自己挑了房客要坞打开，
  不是「设置关掉、恢复上一位」。`设置在场()` 从普通函数改成 `$设置在场 = computed(...)`——它的
  读者里有侧栏按钮高亮（Task 6 接），普通函数不会因为原子变化而让订阅它的组件重渲染。
  `beforeEach` 补上 `$被顶掉的房客.set(undefined)`；`tests/ui/settings-column.test.ts` 新增 7 条，
  钉死 C1、C2、`坞上位()`、以及两条此前的假绿用例。同时改正上一条日志（Task 3 首次落地）里的
  两处失实描述：`开设置栏` 当时并没有做到「只在真的顶掉时才记」——每次调用都会重记，这正是 C1
  的成因；「展开设置时那一列空着」不是 `展开设置` 让坞让位——坞早在 `开设置栏` 那一步就已经被
  顶下去，`展开设置` 只管把窄栏自己收起来，从未碰过坞。
- **Impact**: 行为修正，仍未接入 `App.tsx`（Task 5/6 才会调用这些函数），本次不改变任何已上线的
  可见行为。`state/index.ts` 的转出把 `设置在场` 换成 `$设置在场`，新增转出 `坞上位`——这两个符号
  目前也还没有调用点，重命名不影响任何现有代码。
- **Verification**: 新用例先红：`npx vitest run tests/ui/settings-column.test.ts` → `6 failed | 8 passed
  (14)`，失败的正是 C1、C2、`坞上位` 两条、`$设置在场` 两条（后两条因为 `坞上位`/`$设置在场` 那时
  还不存在）。实现改完后同一条命令 → `14 passed`。`npm run typecheck` 无输出。`npm test` →
  `235 files passed / 2917 passed | 10 skipped`（较 Round 1 多 7 条，无回归）。用 `grep -n "^import"
  src/ui/state/view.ts src/ui/state/right-dock.ts | grep -i settings-column` 确认二者仍不反向依赖
  `settings-column.ts`，无循环。
...
- **What**: 新增 `src/ui/state/settings-column.ts`——`$settingsColumnOpen`（窄栏开没开，不持久化，
  ...
  不是「不知道」）、`设置在场()`、`开设置栏(section?)`（顶掉坞当前房客并记住是谁，坞本来关着就记
  `undefined`——**这一版每次调用都会重记，命令面板反复跳分类会把记忆冲掉，是 Round 2 修的 C1**）、
  `关掉设置()`（窄栏与整页共用同一份收场逻辑，读回被顶掉的房客并复原）、`展开设置()`（把窄栏自己
  收起、切到整页；那一列因而空着——**坞是在 `开设置栏` 那一步就已经被顶下去的，这个函数没有碰过
  坞**）、`收起设置()`（选中项走 `view.ts` 的 `$settingsSection`，两个形状共用同一份事实）。
  `state/index.ts` 转出这一组符号。**未接入 `App.tsx`**：目前没有任何调用点，属于按计划的中间状态。
...
- **Type**: test
- **Motivation**: 第二轮审查过完之后又抓到三处小问题：① 夹具那句「两台，都不关」的注释在这条用例自己开始
  关一台之后已经与代码对不上；② `插件` 那条用例刚定下「按 `.name` 精确取行」这条规矩，`MCP 服务器` 那条自己
  还在用整行子串；③ 插件侧的 `名册变了()` 完全没有用例盯着——审查把 `App.tsx` 退回第二轮修复之前重跑一遍，
  MCP 那条用例红了，插件那条却照样绿，说明那一行新代码今天删掉也不会有任何用例发现。
- **What**: `e2e/settings-counts.spec.ts`——① 重写夹具注释，说清楚「两台起手都开着，故意靠用例自己关掉一台」
  以及为什么（两台都开着时「数开着的」与「数配了几台」分不出来）；② `MCP 服务器` 那一行改成与 `插件` 同一种
  「按 `.name` 精确匹配」写法；③ 插件那条用例改写：先读当前行尾的数（不写死），关掉第一个插件的主开关，
  断言数字相对「刚读到的那个数」掉了一个（不是写死的绝对值，这样以后再加一个内置插件不会无辜变红）。
- **Impact**: 无生产代码改动（`src/ui/App.tsx` 这一轮没有改）。测试覆盖面补齐：MCP 与插件两条口径现在都有
  「真的会因为忘记接 `名册变了()` 而变红」的用例守着。
- **Verification**: 把 `src/ui/App.tsx` 换回上一轮修复之前的版本（`00bdb88` 那一份）、重新 `npm run build`，
  两条用例都如期红了（MCP：`Expected: "1" / Received: "2"`；插件：`Expected: "2" / Received: "3"`，说明起手
  读到的数是 3，关一个之后仍停在 3，不是任何一次都恰好等于 2 的巧合）。换回修复后的 `App.tsx` 重新构建，
  `npx playwright test e2e/settings-counts.spec.ts e2e/mcp.spec.ts --reporter=line` → `9 passed`；
...
- **Type**: fix
- **Motivation**: 作者 2026-08-14 报过、2026-09-14 再撞见、2026-09-16 第三次报：
  *「整个对话的容器好像是两层的容器……你看『回到底部』现在位于哪里了？」*
  截图里那颗浮标浮在半空中。此前两次都没定位（见 memory `pending-sidebar-and-overflow`，
  三轮 e2e 复现全绿、什么都没量到）。
- **What**: 先把作者那张 PNG 逐像素量了一遍（自己写的 PNG 解码器）：两颗滚动条的拇指分别落在
  窗口右缘（`.turns`）和内距里侧（贴底库自己那层），药丸的框是 CSS x 604–706 / y 589–620，
  而输入卡上沿在 823 —— 药丸离它该在的位置**高了 183px**。
  根因：`Thinking()` 里那颗 `<span class="sr-only">正在思考</span>` 是 `position: absolute`
  而**四个偏移量一个都不写**，只能落在静态位置上；静态位置在**包含块**的坐标系里算，
  **不吃中间那层滚动容器的 scrollTop**。而 `.tool` / `.tool-head`（`.btn`）/ `.turn-body` /
  `.turns-inner` 没有一个是定位的，包含块一路逃到 `.turns`——2026-09-08 为那颗浮标刚加的
  `position: relative`。正在跑的那一行长在几千像素长的转录末尾，于是这颗 1×1 的隐藏 span
  把 `.turns` 的滚动区**撑出去 4117px**（探针实测 0 → 4117）。`.turns` 又带着 `overflow: auto`，
  于是它成了真能滚的第二层：滚轮只要没落在里层就去滚它，把同样绝对定位的浮标一起拽上去。
  **09-08 之前 `.turns` 还不是定位祖先，那时包含块是文档**——白出来的是整个 App 页面，
  正是作者 08-14 最早那句话的形状。同一个 bug，换了个受害者，所以它活了一个月。
  两处各修一半：① `.sr-only` 钉住左上角（静态位置永远用不上）；
  ② `.turns` / `.nb-cells` 的 `overflow: auto` → `clip`（`hidden` 仍然是滚动容器，
  代码与聚焦滚动照样能设 `scrollTop`；`clip` 根本不是）。
- **Impact**: 对话区与笔记本各自只剩一层能滚的容器；「回到底部」浮标永远贴着对话区的底。
  只动 CSS，无接口变化。**这条同时关掉了挂起一个月的「向下滑动出现大片空白」。**
- **Verification**: 复现探针（真实构建产物）修前 `.turns` 溢出 0 → 4117、把外层推 200px
  药丸从 741 掉到 541；修后两个数都是 0、药丸回到「底 − 12」。
  新增 2 条 e2e（`stick-to-bottom.spec.ts`）与 2 条设计契约扫描，四条都做过回退验毒：
  把修改改回去立刻红，症状与截图一致（溢出 3435、药丸高出 512 而不是 12）。
  单元 2899 passed / 233 files，typecheck 干净，全套 e2e 485 passed / 1 failed（`remote-kernel` 那条图的用例，
  把改动 stash 掉照样红 —— 本机 matplotlib/内核那条路，与本次无关）。
  **作者在真机上连服务器跑了一轮确认：「这一波的确修建好了，现在来讲完全没有问题了」。**
...
- **Type**: feat
- **Motivation**: 作者截图里一次「看看服务器配置」连着 9 条 bash，每条各自折叠了仍占大半屏：
  *「这些内容，我们能否也进行一个折叠呢？你觉得折叠起来，有必要吗？」* 形状照 Hermes 的 `Ran date … + 1 command`。
  **失败怎么出声是作者选的**：只在汇总行里红字写「N 条失败」，点开才看是哪条（我推荐的是「失败的露在外面」，他没选）。
- **What**（分支 `tool-group`）：
  - `src/ui/tool-group.ts`（纯函数）：`分组转录` —— **两条以上相邻**的工具调用成一组，一条不成组，隔着任何别的条目就断开；
    组 key 取第一条 id（流式时再长一条不重新挂载，展开状态不丢）。`汇总工具组` —— 条数、全是 bash 与否、失败数、
    在跑的是第几条、总耗时（**有一条缺起止时刻就不给**）。
  - `views.tsx` `ToolGroupRow`：收起时 `› ✓ 运行了 N 条命令 · M 条失败(红) 共 X`；混着别的工具说「调用了 N 次工具」；
    在跑时说「正在运行第 k 条：<命令>」并带动画。**收着时组里一条都不画**。展开后是原来的逐条 `ToolRow`——
    **组里的一律先收着、失败的也不自己弹开**（第一版沿用了「报错默认展开」，截图一看三段报错输出一起摊开，与作者选的形状不符）。
    单独一条报错的照旧默认展开。
  - 样式：与 `.tool` 同一副骨架；组里有失败时外侧线**不变红**（红线会把整行说成「失败了」，而它多半是九条错一条）；
    组里成功的逐条不再画自己的灰线，失败的留红线指出是哪条。英文表补 5 条。
  - **mock（准入规则 1）**：`MockToolCallSpec.repeat` —— 前 N 次请求各回一次工具调用，e2e 与夹具共用同一个状态机。
- **Impact**: 只动渲染层；条目、事件流、产物下标（`本轮产物` 仍拿原下标）不变。一轮只调一次工具的会话长相不变。
...
### 2026-09-15 — 上一条修复的回归：`??` 把「我们自己工具的失败」又变回成功（全套 e2e 红 4 条）

- **Type**: fix
- **Motivation**: 上一条把失败判定改成 `e.isError ?? e.result?.isError` 后，**提交前只跑了 `tool-collapse.spec.ts`**。
  全套 e2e 红了 4 条（权限「问一句」拒绝、sudo 硬拒、`⑦` 输入卡权限档、建队选了不存在的模型）：`data-status` 收到 `ok`。
- **What**（分支 `tool-group`）：
  - **根因**：我们自己的工具（远端 bash、权限拒绝、硬拒、建队失败）是**返回**一个带 `isError: true` 的结果，不抛异常——
    那时 pi 的顶层 `isError` 是**明确的 `false`**。`false` 不是空值，`??` 不往后看，结果对象上的 `true` 被盖掉。改成 `||`。
  - 那 3 条用例原本「先点工具行展开、再读输出」——它们是在「失败的从不自己展开」这个 bug 上写的。
    现在失败会自己展开，那一下点击把它收了回去。改成断言 `aria-expanded="true"` 再读输出。
- **Impact**: 恢复上一条之前就对的那四类失败；上一条要修的「pi 自带工具失败」仍然修着（`tool-collapse` 那条仍绿）。
- **Verification**: `codex-polish-2` + `permission` + `team` + `tool-collapse` + `tool-group` 全过。全套 e2e 在下一条（工具组折叠）提交后重跑。
  **教训**：改了事件流上的一个判定，只跑一份用例不够——那个字段全体工具共用。
...
### 2026-09-15 — 对话列跟窗口按比例放大、1200 封顶：784 原本是一个比例，被写成了死数

- **Type**: fix
- **Motivation**: 作者把窗口最大化后发现对话列宽度固定，缩在中间一窄条、两边空一大片（对照 Hermes 铺开）。
  查下来 08-12 量 WorkBuddy 量到的是**占内容区七成**（1113 里 784），但写进 `tokens.css` 的是死数 `784px`。
  作者选「跟窗口按比例放大，封顶」。
- **What**（分支 `fixbug-0915-column`）：
  - `--dawn-thread-max-w: 784px` → `clamp(784px, 70cqi, 1200px)`；`.conversation` 加 `container-type: inline-size` 作参照。
  - **`cqi` 而不是 `%`**：对话层（贴底库的滚动层里，还多一道 `scrollbar-gutter` 占位）与输入卡是两个包含块，
    各取百分比会差出十几像素，三条边就错开了；容器单位让三处（`.turns-inner`、`.composer > *`、`.welcome`）量同一个宽度。
  - 下限 784：普通窗口与从前逐像素一致；上限 1200：行太长回行首会找错行（08-12 那条理由仍成立）。
- **Impact**: 纯样式。窗口宽于约 1120 的对话区才会变宽；视觉基线的窗口尺寸下不变（10 张没动）。
- **Verification**: 先写后改——`e2e/chat-column.spec.ts` 新增「窗宽 1400/2000/3000：不低于 784、封顶 1200、三条边对齐、大窗口真的变宽」，
  改之前红（`784 / 784 / 784`），改之后绿。typecheck 干净；单元 2872 过；视觉基线 9 过 1 红（「命令面板 · 暗色」，
  单跑 `--repeat-each=3` 6/6 过，偶发，未重存）；**全套 e2e 481 passed / 1 skipped / 0 failed**（20.6m）——
  `container-type` 没有把浮层、菜单的定位参照带偏。
...
- **Type**: chore
- **Motivation**: `remote-r` 合并前唯一缺的一步是真集群验一次 R（计划 Task 6）。
- **What**: 作者在自己的集群（`gs191`）上用合完 main 的那一版开远端会话，内核在服务器的 `/pub/R/bin/R` 起来并跑了 R。
  原话：*「remote-r 应该是好用了，我现在已经测试过了。」* 随后 main 从 `f46e2ce` **快进**到 `remote-r`，无合并提交。
  计划 Task 6 两格打勾。
- **Impact**: 远端 R 进入 main。**未推**（作者自己推）。
- **Verification**: 快进前那一版的单元 2872 / e2e 480 全绿（见下一条）；真机由作者验。
  **验的途中撞见一次旧 bug**：等回话时往下滚，转录区出现大片空白、两条竖滚动条；等待结束后用 DevTools 探针量，
  只有贴底库那层在滚、`.turns` 不溢出，随后自己恢复。间歇性、未定位、没修，与远端 R 无关，记在挂起的那条 bug 名下。
...
- **Type**: fix
- **Motivation**: 整条分支的终审（read-only reviewer）给了 7 条 Important、8 条 Minor。
  **逐条核实过再动，不照单全收**——15 条全部成立，M3 是已知的不精确，照记不修。
- **What**:
  - **I7 毛玻璃整条分支上一处都没生效。** 7 个浮层加了 `backdrop-filter`，底却仍是不透明的
    `elevated`——模糊压在实心板后面。作者选 B 时看到的「透过玻璃看见后面」，当时界面上一处都没有。
    新增 `--dawn-surface-overlay`（`elevated` 兑 84% transparent），7 个浮层换上，降级块跟着改。
  - **I5/I6 认错了元素**：`.dock-content` 是**底部终端坞**，不是右侧坞。xterm 自己涂侧栏色，
    玻璃在终端四周围出一圈色带。容器上的「玻璃」（`--theme-surface-glass`、`--dawn-blur-panel`）
    **整个撤掉**——没有模糊、底色是平的，它兑出来和原来那档一样，名存实亡。
  - **I1 导出提示在减弱动效下只显示 0.3 秒**（我在第三轮带出来的）。它由 `onAnimationEnd` 卸载，
    **动画时长就是停留时长**，我写的 `animation: … 300ms` 把 4.5s 砍没了，而那是导出唯一的结果。
    改成只换 `animation-name`，时长不碰。
  - **I3 打开 200 条的会话，200 条一起动。** 进场动画挂在 `.turn` 上，而对话视图按会话挂载。
    改成 `.turn:not(:has(~ .turn))`——只有最后一条动；流式输出时不重播。
  - **I2** 设置里「无框」变体漏清新加的影子；**I4** 主题测试丢 alpha、对半透明侧栏失明（改为合成后再比）；
    **M6** 表格与 PDF 的子元素会盖住 inset 描边，换回「真描边 + `md-bordered`」；
    **M7** 假发布源的 error 监听在 listening 后没摘，启动后的错误会被吞；**M8** 一处控件三态误用了新曲线。
  - **M1/M2 两条扫描收紧**：no-border 不再只看行首（一行写完的规则原先会漏）、认单边长写法；
    reduced-motion 改成整段类名匹配（原先 `.menu-scrim` 能替 `.menu` 顶包）、认 `translate/scale/rotate` 与 `animation-name`。
  - 顺带修一条 e2e 的等待条件：`files.spec`「坞宽过了窗口能给的」先等坞、再**立刻**读按钮，
    而坞跨过两栏阈值时面板会重建一次——列宽过渡缩到 180ms 后撞上了那一帧。改成直接等按钮。
- **Impact**: 纯视觉层；边界（protocol / main / state）终审确认未动。10 张视觉基线第四次重存。
- **Verification**: 单元 **2854 全绿**、typecheck 干净、设计契约 54 条；两条收紧的扫描各拿探针验过会红。
  视觉基线重存后**整份 5/5、两张浮层屏各单跑 5/5**——模糊是确定性的（对照：渐变那版同一屏 6 次红 5 次）。
  **全套 e2e 479 passed / 0 failed**（20.6m），`kernel-session` 另跑 **6 passed**——
  与 main 上 09-09 那一轮的基线（479 / 0）完全一致。

  **这一轮最该记住的**：前面三轮，审查员两次交回的报告都没送到，我以为是"没问题"。
  这次让它把结论写进文件才拿到——**15 条里有 3 条是会被用户直接看见的**（玻璃不生效、终端色带、0.3 秒的提示）。
  **收不到报告不等于报告说没事。**

---

### 2026-09-11 — 视觉重做第三轮：排版跨度与动效；应用底撤回平色

- **Type**: feat
- **Motivation**: 作者说「简陋」，诊断出三个原因：深度、排版跨度、动效。第二轮只解决了第一个。
- **What**:
  - **字号**：59 处裸 `font-size` → 3 处（全是有理由的豁免：WorkBuddy 量来的侧栏行 14px、
    欢迎页字形 28px、用量图刻度 9px）。53 处 11px 收编进 `--dawn-fs-badge`，
    字号扫描那个开了三周的 `(11|10|9)px` 口子关上了。
  - **分组小标**：5 条改成 10px 大写宽字距。**第一份名单是错的**——按名字选，
    漏了侧栏的「会话」「服务器」（`.side-head`，产品里最显眼的那个），
    又收进两个正文号的小标题。判据是这行字长什么样、在说什么，不是它叫什么。
  - **会话标题** 14px → 22px，全仓唯一用 `display` 的地方（定义了没人用的档位等于没拉开）。
  - **动效**：3 条非热路径过渡换 `--dawn-ease`，其余 7 条控件三态照旧 `ease`；
    新消息 4px 进场，reduced-motion 下**关位移、留淡入**（与本仓库既有做法一致，不一刀 `none`）。
  - **应用底撤回平色**（作者定的，见下）。
- **Impact**: 纯视觉层，功能零改动。10 张视觉基线第三次重存。
- **Verification**: 单元 **2854 全绿**、typecheck 干净。
  视觉基线重存后**整份 9 次里红过 1 次（现场没留下），另 6/6 单跑全绿**——
  如实记下，不写成"稳定"。

  **应用底为什么撤回**：极淡渐变与逐像素基线不能共存。它的抖动相位会在两种**确定的**
  状态之间跳——「命令面板·亮色」**6 次红 5 次、每次恰好 59558 像素**，同一屏换回平色
  **6/6 稳定**。作者在「留渐变」与「留基线」之间选了基线。损失的效果按设计本来就要
  "小到说不出它是渐变"。**别再把渐变加回这一层，除非先拿出一个在基线上稳定的做法。**

  **看图抓到一条**：`10px + semibold` 的中文比原来 `11px` 常规的**还糊**——
  加粗把字腔填死了（5 倍放大，「展」字糊成一团）。规格给 label 的定义里本来带着 `600`，
  等于一边写「别为中文加粗」一边把加粗写进了定义。去掉，小·灰·宽三个信号够了。

  **新扫描：带位移的动画必须能关，纯淡入淡出豁免**——DESIGN.md 那句话的自动化形式。
  计划原本写的是「用了 `@keyframes` 就必须有 reduced-motion 块」，**文件级判断**，
  而这文件本来就有 8 个这样的块，**它天生就绿、永远抓不到东西**。改成逐个动画判之后
  当场抓到一条先有的缺口：`.export-toast` 带 `translateY(-4px)` 却没有出路。

  **§9 的伪问题**：规格说会话标题是"新长出来的元素，会吃掉约 60px"，要不要滚动收起。
  **标题早就有**（`.conv-title`）。量出来头栏只高了 **4px**，常驻。
  那句"新长出来"是我没查就写的——**不在纸上拍板是对的，但也别在纸上凭空描述现状。**

---

### 2026-09-11 — 视觉重做第二轮：面的归位（一根线 → 浮起来）

- **Type**: feat
- **Motivation**: 第一轮把令牌定好了但没人用。这一轮把调用点换过来——
  **「面与面之间只有一根线」才是那份简陋感的机制**。
- **What**:
  - **22 个内容面换 elevation**。判据（四边俱全 + 装的是内容）实际做下来**漏了一件事：
    描边是什么颜色**。5 个中性灰的删线换 `--dawn-shadow-md`；**16 个是主题色描边**，
    那是作者 2026-08-23 定的（*「白色背景 + 绿色的线」*），不是随手写的分层线。
    作者看过亮暗三版对照后选「两个都要」，于是有了第五档 `--dawn-shadow-md-bordered`
    ——`md` 去掉 `inset 0 0 0 1px`，由绿边勾形状、阴影给厚度。
  - **侧栏与坞半透明、浮层加模糊**，降级块在 `tokens.css`（改的是种子层令牌，种子只有一个家）。
  - **应用底加极淡渐变**（`background-color` 不接受渐变，会静默失效，调用点一并改）。
- **Impact**: 纯视觉层，功能零改动。10 张视觉基线全部重存。
- **Verification**: 单元 **2853 全绿**、typecheck 干净、**非视觉 e2e 468 passed**、
  视觉基线重存后**连验三轮全绿**。

  **审查抓到 0 条，e2e 抓到 2 条，看图抓到 1 条。** 逐条记下来：

  1. **玻璃往亮里兑，把「侧栏最深」翻了过来**（`theme.spec` 那条「层次靠的是这个，
     不是装饰」抓的）。`tokens.css` 里那句「侧栏最深、内容区抬起」是暗色层次的骨架，
     而我照着 macOS 那种"侧栏更亮"画的对照图、作者照着它点的头。
     **玻璃靠的是"透 + 糊"，不是往哪边兑**，改成兑黑，两者都保住。
  2. **`backdrop-filter` 让侧栏成了它 `position: fixed` 后代的包含块**
     （`sess-title` 那条「窗口矮的时候…」抓的，全套里只有它红）。悬停卡就长在侧栏里，
     它用 `fixed` 是硬要求（侧栏会滚，绝对定位会跟着列表走，**本仓库栽过一次**）。
     **一个纯视觉属性废掉了一条有注释、有前科的定位契约。**
     而那个模糊在那儿**本来就没做事**——布局容器背后只有 body 的渐变，
     模糊一个平滑渐变得到的还是同一个渐变。已加扫描：带 `backdrop-filter` 的规则
     必须同时有 `--dawn-shadow-float`（浮层在这儿都是这么写的），加回去当场红。
  3. **灯箱里的图被加了模糊**：`backdrop-filter` 糊的是图片**背后**，而那行不透明的
     `background` 本来是给**带透明通道的图**垫底的——这个应用天天在看 matplotlib 出的图。

  **三条新扫描是空真的（调用点还没出现），所以逐条拿探针验过会红**：
  md/lg 的 no-border 扫描精确抓到 `6667: .dawn-scan-probe`；毛玻璃两条连同既有的
  「无裸色值」一起红。**一条从没在眼前红过的扫描，和一条摆设是分不出来的。**

  **基线重存踩了记着的那个坑**：`--update-snapshots=all` 之后**两张稳定红**。
  先验了渲染是不是确定的（同屏两次渲染**字节完全相同**，是确定的），
  于是按 CLAUDE.md 记的办法救——**把验证轮的 `-actual.png`（稳定化之后的帧）拷成基线**，
  没有再 update。**另记一条**：整份视觉用例在**本机开着 `dev:mock` 时会偶发红**
  （概览·亮色，3 次里 1 次，9% 像素），关掉之后 3/3 全绿；单跑 6/6 全绿。
  这与 09-09 记的「满负载下的抖动」是同一类，**下次看到别急着重存基线，先看机器上还开着什么**。

- **另修一条与视觉无关的真 bug**：`dev:mock` 崩在
  `fake-release-feed.mjs` 的 `server.address().port` —— `listen()` 是异步的，
  `address()` 在 `listening` 之前是 `null`。**这个竞态平时会赢**，所以活了很久：
  四个调用点里两个写着 `await setTimeout(200)` 盖过去，另外两个
  （`dev-mock.mjs`、**`e2e/fixtures.ts`**）连这个都没有。
  **睡 200ms 不是修好，是把赢的概率调高一点。** 现在给了真的就绪信号，
  四处全部 `await`，两个 sleep 删掉，早读端口会抛一句说人话的错。
  `e2e/fixtures.ts` 也在这条路上——**历史里那些「偶发红、单跑又绿」可能有一部分是它**。

---

### 2026-09-10 — 视觉重做第一轮：令牌层，判据是「基线不许动」

- **Type**: feat
- **Motivation**: 规格 `2026-09-10-视觉重做-design.md` 第一轮。补上缺的深度、
  排版跨度、动效三样的**定义**，一个调用点都不碰。
- **What**: `tokens.css` 新增 13 个令牌，`design-contract.test.ts` 新增 6 条扫描。
  - **深度补 md / lg 两档**，亮暗各一套。亮色沿用 float 已确立的形状
    （单一顶部光源、x=0、负扩散随模糊增大）；暗色走 `--dawn-shadow-sm` 那条路
    （一圈描边在阴影的 `inset` 里），**外加一道方向性顶边高光 `inset 0 1px 0`**——
    一整圈一样亮读起来是「有边框」，只有上边亮才是「有厚度」。
  - **毛玻璃两档**（`blur(14px) saturate(150%)` / `20px 180%`）。全仓此前 `backdrop-filter` 0 处。
  - **字号从四档补到七档**，跨度 18/12 = 1.5× 拉到 22/10 = **2.2×**。
    新的 `--dawn-fs-badge` 不是新档位，是**一个一直没有名字的旧角色**
    （四档那版注释写着「11px 只给徽章与胶囊」，于是它被写死了 53 次）。
  - **一条缓动曲线 + 三个时长**。此前 10 条 `transition` 清一色 `ease`——浏览器缺省，
    等于没做过动效决定。
- **Impact**: 纯定义，**`styles.css` 整轮一个字没动**，功能零影响。
  `docs/DESIGN.md` 契约表已登记全部新令牌。
- **Verification**: `npm test` **2852 条全绿**、`typecheck` 干净、
  **视觉基线 10 张全绿且文件零字节改动**——这一轮的判据是反的：新令牌还没人用，
  基线动了就说明改错了地方。

  **三条扫描是空真的（`styles.css` 里还没有调用点），所以逐条拿探针验过会红**：
  md/lg 的 no-border 扫描精确抓到 `6667: .dawn-scan-probe`；
  两条毛玻璃扫描连同既有的「无裸色值」一起红。
  **一条从没在眼前红过的扫描，和一条摆设是分不出来的。**

  **审查抓到两条真缺陷，都是规格的数值错，不是实现错**：
  ① `lg` 的远层 blur 32 比 `float` 的 28 还大——**梯子搭反了**，
  浮层开在输入卡上面时上层阴影反而更散，就不像浮在上面。改成
  `0 10px 24px -6px / 8%`，四档现在 blur `2/16/24/28`、alpha `6/6/8/10` 两个维度都单调。
  ② 暗色注释里写着「有扫描拦着」，而那条扫描原计划在第二轮，
  **第一轮又是能单独合并的**——中间存在一个「注释在说假话」的版本。
  扫描提前到第一轮。这个仓库为这种事付过代价（`window.prompt` 那条规则）。

  **顺带修正了计划里第二轮的一个坑**：毛玻璃的 `@supports` 降级块原定写进
  `styles.css`，但它重新定义的是**种子层令牌**，那会让这个值有两个家
  （`--dawn-code-size` 就这么翻过车：量表记 13px、界面上一直是 12px）。
  改成**模糊用在 `styles.css`，降级归 `tokens.css`**。

---

### 2026-09-10 — 视觉重做定案：缺的不是配色，是深度

- **Type**: docs
- **Motivation**: 作者说*「界面非常的简陋，没有那种高级的感觉」*，并指了
  `/Applications/PlotCase.app` 问这个风格怎么样。
- **What**:
  - **读了 PlotCase**（Electron，`app.asar` 里的 `renderer/*.css`，150KB）。
    结论是**版面与色值体系都不学**：它是模板画廊不是 agent 工作台，
    **695 个互不相同的十六进制色值、零令牌层、`prefers-color-scheme` 0 次**。
    只拿两件：较新那层 `.dc-*` 的两层阴影形状、`cubic-bezier(.22,1,.36,1)`。
  - **量了自己**，确认「简陋」不是配色问题：`backdrop-filter` **0 处**、
    `--dawn-shadow-sm` 被调用 **1 处**、`box-shadow` 25 处 vs `border:` **62 处**、
    应用底平色、`transition` 10 条全 `ease`、字号跨度 **1.5×**（用得最多的是 12px，116 处）。
    **根因一句话：面与面之间只有一根线**——深度语汇写得很好，但只用在浮层上。
  - 浏览器里并排看了四个方向（沉静克制 / 材质与深度 / 排版驱动 / 科学仪器），
    作者选 **B 材质与深度**，并在亮暗真实尺寸对照后确认。
  - 规格落在 `specs/2026-09-10-视觉重做-design.md`：深度补 md/lg 两档、
    毛玻璃两档（带 `@supports` 降级）、字号加 display/label 两档把跨度拉到 2.2×、
    一条缓动曲线三个时长。**分三轮**（令牌层 → 面的归位 → 排版与动效）。
  - 新增三条可判定规则进 `design-contract.test.ts`（CLAUDE.md 第 2 条）。
- **Impact**: 纯视觉层。**功能一行不动**——`src/protocol/`、`src/main/`、
  `src/ui/state/` 与内核/远端/会话逻辑全部在边界外。
  颜色体系（13 档灰阶、70/50/30、描边兑透明）**不重开，它是对的**。
  ②③ 两轮会让 10 张视觉基线全红，那个红是预期的。
- **Verification**: 本轮只有文档。**一处自我纠正记在这里**：写规格时先断言
  「暗色换阴影机制需要给 `.dawn-dark` 破例」，查证后是错的——
  `ALLOWED` 白名单里 `--dawn-shadow-` 早在列，暗色 `inset 0 0 0 1px 白5%` 也早已落地
  （2026-08-23，学自 WorkBuddy 的 `#ffffff14`）。**没有规矩要改**，
  这一轮做的是把已有机制铺到中间档，外加一道方向性顶边高光。
  规格自查还砍掉了一个 `--dawn-shadow-xs`：它与 `sm` 只差 1%，
  屏幕上分不出来，正是「两处长得一样的东西等于没有判据」。
...
- **Type**: fix
- **Motivation**: 作者问：*「优化输入里面的逻辑，是不是基于我们选择的不同的 LLM 而设置的，
  应该不是锁死在某一个 LLM 的吧？」*
  查下来**没锁死**：native 会话走的是 `NativeRuntime.问一句({ sessionId })` →
  `s.session.model`，就是模型 pill 上那个，换模型换服务都跟着换。
  **但有一条他不知道**：cli（claude / codex）、ACP 会话与空态屏**够不着自己的模型**
  （那些模型在别人的进程里），只能借配置里第一个 native——
  **而界面对此一个字都不说**。后端早就把 `model` 回过去了，界面把它扔了。
  「不静默降级、不静默换人」（规格 7.5）管的正是这一类。
- **What**:
  - 协议 7.33：`enhancePrompt` 的响应多一个 `borrowed`（纯新增，故 minor）。
    后端算 `借的 = !(rec && kind === "native")` 一并回给界面。
  - 界面新增纯函数 `说这一次`：把「谁改的」与「带了什么上下文」拼成那一行灰字。
    **只在借了的时候说谁改的**——没借时那就是你屏幕上那颗 pill，再说一遍是纯噪音，
    而噪音多了真正该被看见的那一句就沉下去了。
  - 抽成纯函数是因为它是一条**可判定的规则**，不该只能靠打开界面去看。
- **Impact**: 在 claude / codex / ACP 会话里点「优化输入」，下面那行灰字会说清
  「这段对话的 agent 没有可直接调用的模型，用「deepseek/deepseek-chat」改的」。
  native 会话的表现一个像素都没变。
- **「第一个 native」按配置顺序挑——这是决定，不是遗漏**（作者当场定的）：
  摆过两条别的路（「你上次用过的那个 native」、设置里单独指定一个），
  他的话是*「按照配置的先后顺序吧。不用继续往前了」*。
  理由站得住：**挑到哪一家现在会如实说出来**，而「说出来」正是让这个简单规则
  可以一直简单下去的那件事。已把这段写进 `backend.ts` 那行 `find` 的头上——
  没有它，那行代码看起来像个待修的 bug，迟早被人顺手「修」掉。
- **Verification**: `tests/ui/enhance-note.test.ts` 六条盯那条规则（借了要说 / 没借不许提 /
  两件事拼一行 / `note` 压过 `usedContext` / 什么都没有就不留空灰字）。
  `e2e/enhance.spec.ts` 新增两条，跑真产物：空态屏借了要说、native 会话一个字都不提。
  **红那一轮是自己量的**：把那句从 `enhance.tsx` 摘掉、重新 build——
  「借了要说」当场红，而「没借不许说」照旧绿（第二条是独立的闸，不靠第一条撑绿）。
  恢复重建后 8 条全绿。typecheck 静默。

---

### 2026-09-08 — 跟随撒手之后，出路要看得见：回到底部浮标（顺带修好一条瞎了的扫描）

- **Type**: feat
- **Motivation**: 作者：*「分析的内容不更新，需要我手动往下挪动。」*
  同一天已修掉一条真根因（丢掉的 `mouseup`，见下一条记录），但那句话里还剩一半没被回答——
  **整个界面里没有任何一个「回到底部」的入口**（`scrollToBottom` 一处都没用过）。
  `use-stick-to-bottom` 撒手本身是对的（你翻上去看前面，不该被下一个 token 弹回来）；
  缺的是「撒手了」看得见、且一键回得去。「看不见的能力等于不存在」——
  2026-08-10 一天之内为这条栽过两次。
  规格 `specs/2026-09-08-回到底部浮标-design.md`，计划 `plans/2026-09-08-回到底部浮标.md`。
- **What**:
  - 新增 `src/ui/back-to-bottom.tsx`。三个状态：贴底**一个像素都不画** /
    没贴底说「↓ 回到底部」/ 底下长出过新内容就改口说「↓ 有新内容」。
    判据抽成纯函数（`该说什么`、`下一个长过`），理由同 `carryDraft`：
    **它发生在一场竞态里，而规则本身是确定的，可以直接验**。
    `该说什么` 的**返回值就是那句话**——两个可见状态的差别只有文案，
    那就让判据直接把文案给出来，别在组件里再翻译一次。
  - **形状换过一次（2026-09-09）。** 第一版按作者当时选的做成「右下角一颗圆的 `↓`」，
    他看到实物后说*「这个按钮太丑了，我们做成方案1设计的样式吧」*——方案一是当初提的
    第一个选项：**底部居中的带字药丸**。三个选项当时都配了 ASCII 图，他仍然是看到真东西
    才改的主意：**版面形状的选项要配图，而图不如实物**。
    换过来还白捡一件事：设计契约禁原生 `title`，那颗光秃秃的圆点因此悬停什么都不出，
    只有读屏知道它是干嘛的；**带上字之后可见文字就是它的可访问名**，
    连 `aria-label` 都不需要了。
  - **「底下长出新东西」由组件自己的 `ResizeObserver` 盯 `contentRef` 得出，不让调用方传记号**：
    流式回复时条数不变、内容却在长——传条数会漏掉最常见的那一种，而那正是作者报的场景；
    对话区与笔记本因此是同一份代码、零配置。
  - 放在 `<StickToBottom>` 里、`.Content` **外面**（那一层不滚，2026-09-08 在真产物上量过）。
    对话区与笔记本各一颗——同一条规则不许只套一半（粘贴图片当初只给了对话那个输入框，
    而应用打开就落在空态卡上，2026-08-13 吃过一次）。
  - 几何住进 primitive 新的一档 `size="pill"`（32px 正圆，有字自己撑开），
    `.stick-pill` 只剩「浮在哪儿 + 浮起来的那层面」——同 `card` 那一档的原话：
    **多一档尺寸，几何就仍然只有一个家**。
- **顺带修好一条瞎了的扫描（这一轮最值钱的部分）**：
  - 审查与我各自独立抓到浮标第一版的两条违规（裸 `<button>`、原生 `title=`，`docs/DESIGN.md:34-35`）。
    顺着查下去发现**那条扫描本身是瞎的**：它逐行匹配 `/<button[\s>]/`，而属性一多 JSX 就被
    格式化成每行一个属性，`<button` 落在**行尾**，后面既没空白也没 `>`。
    这是本项目第三次栽在「逐行扫描抓不住跨行 JSX」上（emoji 那条、按钮子串那条，现在是这条）。
    改成**整篇匹配**，并且**先把注释抹成等长空格再扫**（不抹的话，一条解释「为什么不许写裸
    button」的注释自己就成了违规——改完第一分钟就撞上了；抹成空格是为了行号不变）。
  - 修好之后浮出**四处既有违规**：`Settings.tsx` 两个主题色色块 → 给一条**按类名的窄豁免**
    （纯色块 radio，套 primitive 就得从调用点改写内距圆角，那违反另一条更硬的规则；
    **豁免按类名给不按文件给**，否则那个文件里以后新写的裸按钮就再没人看着了）；
    `agent-browser.tsx` 历史行、`session-tabs.tsx` 关闭 ×、`turn-navigator.tsx` 刻度
    → 三处都改走 primitive，照 `Row` 那条既有办法（`size="inline"` 清零盒子，几何仍归各自的类），
    `agent-browser` 那处的原生 `title` 一并去掉。
- **Impact**: 翻上去看历史之后不再只能手动拖回底部；正在跑的时候翻上去，浮标会改口说「有新内容」。
  贴着底时一个像素都不画。两个状态都带可见文字，读屏与眼睛看到的是同一句话。
- **Verification**:
  - `tests/ui/back-to-bottom.test.ts` 九条盯判据（含一条「两个可见状态说的话不一样」——
    两处长得一样的东西等于没有判据，这个项目一天之内为它咬过三次）。
  - `e2e/stick-to-bottom.spec.ts` 新增三条（贴底不画 / 点它回底且跟随恢复 / 新内容到达时带字），
    **红那一轮是自己量的**：把 `<回到底部 />` 从 `views.tsx` 摘掉、重新 build、跑真产物——
    两条盯浮标的当场红（`.stick-pill` 找不到），而那条「贴底不画」照旧绿
    （说明它确实只是道闸，不是靠功能撑绿的）。恢复重建后 5 条全绿。
    换形状之后又补了两条文案断言（常规那一下必须说「回到底部」），仍是 5 条全绿。
  - **i18n 那条扫描当场咬了一口**：把中文原文写成变量传进翻译函数，英文表里那两条
    就成了「谁也不用的孤儿」。改回 `enhance.tsx` 里 `档名` 那张表的写法——
    字面量留在源码里。（顺带记下：那条扫描**也没抹注释**，与 `design-contract` 里
    裸 button 那条同一类；区别是它只会假阳性、不会漏判，所以这次没顺手改它。）
  - **三处 primitive 转换一个像素都没动**：它们各自的类本来就完整拥有几何，
    而且在样式表里排在 `.btn*` 后面。
  - **但十张基线里有两张确实重存了：`设置-亮色/暗色`**——**跟本轮无关，是补记**。
    逐像素比过旧新两张（`290` 个不同像素，全落在设置左栏那一列上）：
    `Skills` 从 `4` 变 `5`（`1a1cbb7` 那个「照着文献画图」技能加进去的），
    并且多了一行**「关于」**（应用内更新那轮的）。也就是说这两张在 `1a1cbb7` 上
    **本来就是红的**——两次该重存的时候都没重存，欠到了这一轮。
    **`Skills N` 这个数跟着仓库里的技能条数走**：下次加技能它还会红。
    按「外面世界的东西不进逐像素基线」，它该被 `mask` 掉——本轮没做，记在这儿。
  - 设计契约 45 条、i18n、单元全量、typecheck、全量 e2e。
    （`tests/kernel/*.integration` 那几条真内核用例在满负载下会偶发 90s 超时，单独跑 2 秒即过——
    与本轮无关，两次都验过。）

---

### 2026-09-08 — 「优化输入」跟着那句话一起作废；丢掉的 mouseup 不再让对话停在半空；`keyCode 229` 不再单独说话

- **Type**: fix
- **Motivation**: 作者一次报了三条，这一轮修掉其中两条（第 ① 条「中文打一半变英文」
  还没复现出来，见末尾）：
  - ②*「优化输入……其实是单次的，优化结束之后，我发现优化按钮并没有重置。
    然后对话提交上去了，竟然还[能]撤销，我撤销之后竟然还是原来的对话。」*
  - ③*「有时候我鼠标不在 DAWN 了，分析的新内容竟然不往下弹，
    我需要点击之后才会弹到最新内容。」*
- **What**（分支 `fixbug-0908-input`）：
  - **②：撤回栈的作用域写错了。** 它是 `EnhanceControl` 里的一个 `useState`，
    而它描述的其实是**框里这一版草稿**——那句话发出去之后栈还留着，
    按下「撤回」就把**已经发走的那句话的上一版**填回了空框。
    新增 `重置记号` 由调用方声明作用域（学 `$drafts` 那条：作用域要写进 key 里），
    两张输入卡在提交那一刻各换一个值；同时把「带上了什么」那行说明一起清掉。
    **同一条缺陷还有另一半**（第二轮补的）：不只撤回栈，**还在飞的那一次改写**也属于
    这一版草稿。上一版特意写了「只清栈、不碰改写中」，理由是那件事有自己的终点——
    可那个终点会 `setDraft(r.text)`，**写进的是下一版草稿**：点了「优化输入」
    紧接着按回车，话发出去、框清空，8 秒后那句话的**改写版**自己冒回空框里，
    旁边还立起一颗「撤回」。现在 `重置记号` 一变就把在飞的请求一并作废并 `cancel`。
  - **③：根因在依赖里，是一个模块级全局。** `use-stick-to-bottom@1.1.6` 开头
    `let mouseDown` 只在 document 的 `mouseup` / `click` 上清，**而这两下是会丢的**
    （右键弹系统菜单、拖到窗口外松手、按着时切走应用、HTML5 拖拽根本不发 mouseup）。
    丢掉之后只要转录里还有选区，`isSelecting()` 恒真，贴底动画每帧 `return next()` 空转——
    下一次点击把标志清掉才「唰」地补上。1.1.6 是上游最新版，没修过这条。
    新增 `src/ui/mouse-stuck.ts`：**只在能证明没有按键的四个时刻**补发一次 `mouseup`
    （`mousemove` 且 `buttons === 0`、窗口 `blur`、`contextmenu` 的下一拍、`dragend`/`drop`），
    且只有我们自己见过一次没配对的 `mousedown` 才补。装在 `main.tsx`：
    它修的是模块级全局，寿命是「这个窗口活着」。
- **Impact**: 发完一句话，「优化输入」旁边不再留着一颗会把旧话灌回来的「撤回」；
  agent 在跑、人的鼠标不在 DAWN 上时，新内容照旧跟到底。**人真的按着鼠标划词时仍然不抢视线**——
  那是这类修法最容易顺手弄坏的东西，配了用例盯着。
- **Verification**（**先量后改**）：
  - ③ 的根因是量出来的：探针在真产物上复现——新内容到达后离底 **228px**，
    「点一下」之后 **1px**。那条探针已改成常驻用例 `e2e/stick-to-bottom.spec.ts`，
    两条：丢了 mouseup 也要跟到底（修前红、修后绿）、真按着鼠标时不许抢视线（一直绿）。
    顺带记下一件量出来的事实：**真正在滚的是 `.turns > div`**（`StickToBottom.Content`
    自己造的那层），不是我们写了 `overflow:auto` 的 `.turns`——量错元素会得到一组
    「永远贴底」的假数字。
  - ② 配 `e2e/enhance.spec.ts` 两条，都是**先红后绿**（红那一轮是把修改从
    `enhance.tsx` 里摘掉、重新 build、跑真产物量出来的）：发出去之后「撤回」必须消失、
    框必须是空的；以及「点了优化紧接着回车，8 秒后改写的结果不许再落进空框」。
  - ① 配 `e2e/ime.spec.ts` 第三条：**输入法把每一下都报成 229 时，回车照样发得出去**。
    这一条在改之前是**红的**（真产物上量的：话根本发不出去，`.conv-title` 20 秒没出现），
    改完绿。另加 `tests/ui/ime.test.ts` 六条盯着判据本身
    （组词中的 229 算、组词外的 229 不算、上屏之后不算、`focusout` 之后不算）。
  - `tests/ui/mouse-stuck.test.ts` 九条盯着补发判据（该补的补、按着不补、补过不再补、卸载即停）。
  - 单元 + typecheck + 全量 e2e。
- **顺手补的两张视觉基线**：设置屏（明暗各一张）自 2026-09-06 起就是旧的——
  多了「关于」那一行（应用内更新那一轮加的）、Skills 计数 4→5（`figure-like-this` 那一轮加的）。
  **先看 diff 图确认这两处都是真变化**才重存。`--update-snapshots=all` 照例把
  命令面板那张写成了未稳定化的一帧（CLAUDE.md 里记着这个坑），按记着的办法用验证轮的
  `-actual.png` 覆盖回去，再验两遍全绿。
- **第 ① 条：**`keyCode 229` **那条假阳性已修；作者报的那个症状仍未复现。**
  *「打到了 zh 之后就不能继续打中文了，输入法是中文，但是被迫变成英文了。」*

  - **修掉的那一条（真的、量得出来的）**：`src/ui/ime.ts` 的判据原是
    `isComposing || keyCode === 229`。第二项的用意没错（有些平台被输入法吃掉的键
    只报 229），**但它会假阳性，而且是常见的那一种**：
    - macOS 拼音**候选词已经上屏之后**的那一下回车，仍可能带 `keyCode=229`
      而 `isComposing=false`；
    - 豆包 / 搜狗这一类第三方输入法**每一下按键都报 229**，英文模式下照报
      （xterm.js #5887 量到的就是这个）。

    后果是我们把**每一下按键都让给输入法**：回车不发送（只换行）、Esc 中断不了、
    ↑↓ 翻不了历史、`@` 与 `/` 菜单的键盘全不响——症状与「输入法坏了」一模一样，
    而代码是我们的。现在 229 **只在我们确实看见 `compositionstart`、还没看见
    `compositionend` 的时候**才算数；那面旗子由 document 上的组词事件同步维护，
    **不带定时器**（用 `setTimeout` 延迟清标志正是同类应用把输入法卡死的第三号根因），
    并且 `focusout` 也当作组词结束——漏收这一条旗子会永远卡住。
    七个调用点（两张输入卡、改一句话、命令面板、文件跳转、记忆、网址栏、强调色）
    一起受益。

  - **作者那个症状仍未复现，而且这一轮又排除了一大类。** 这一轮的探针在真产物上
    **连续组词 40 秒**（`z→zh→…→zhongwen` 循环 40 遍），两张屏各跑一遍——
    对话屏跨了一整轮流式回复，空态卡跨了 8 次 5 秒刷新——记录里：
    **0 次 focusout、0 次节点重挂、0 次 `disabled` 翻转、0 次程序改写值、
    0 次意外的 `compositionend`**（320 次采样只反映我们自己打进去的那些字）。
    加上上一轮已排除的「主线程堵住」，**界面这一层现在是干净的**。
  - **作者补的复现条件：*「是不是 ssh 和服务器对话的时候发生的问题呢？」*** 照着量了——
    对着那台假 SSH 服务器起一段远端会话、发一句、**连续组词 40 秒**（跨流式回复 + 8 次
    5 秒刷新）：324 条记录里**一条异常都没有**（唯一那条 focusout 是探针自己点侧栏时的）。
    再加一次**组词途中把 SSH 掐断**（第 4 轮中间 `disconnectRemote`）：输入框照旧
    有焦点、没被禁、节点没换、占位符没变，组词毫发无损。**远端这条路也是干净的**——
    但要说清楚：假服务器不会自己抖，真机上那一下断线是什么样，这条量不到。
  - **作者机器上的输入法已经查清**：`AppleEnabledInputSources` 里只有 **ABC** 和
    **系统简体拼音 `com.apple.inputmethod.SCIM.ITABC`**，没有搜狗 / 微信 / 豆包。
    所以上面那条 229 的修法虽然是真缺陷，**多半不是作者撞的那一条**。
    另记一件事实：`AppleCapsLockPressAndHoldToggleOff = 0`，即**轻按 Caps Lock 就在
    中/英之间切**——「自动变为英文」在这台机器上长得就是这个样子。
  - **一条听起来很像、但量下来是假的**：① 和 ③ 的解药都是「点一下」，所以试过
    「同一个丢掉的 `mouseup` 也解释 ①」——若 Blink 也认为鼠标还按着，每次移动都在扩选，
    而选区一变组词就会被取消。探针照着做了（在转录的文字上按下不松手 + 连续移动，
    再去组 `z→zh→zho`）：**组词毫发无损，文档选区始终是空的**。这条否掉。
  - 剩下的差别只有一处：**CDP 的 `Input.imeSetComposition` 走不到 macOS 那条真通路**
    （NSTextInputClient ↔ 渲染进程）。同类应用报过的三号根因也都只在真通路上发生：
    组词途中被程序 `focus()`、组词途中被程序改写值、用定时器延迟清组词标志。
    我们第三条没有；第二条只剩 `写回` 那个 `requestAnimationFrame` 里的
    `setSelectionRange`（只在 `@` 菜单选完之后那一拍）；第一条只有挂载时的 `autoFocus`。
  - **收尾**：这一版从源码 build 出来打开给作者试，他的反馈是*「其实我现在发现，
    又没有问题了」*。**这不算「已修」**——原报告本来就是「有的时候」，而这一轮
    没有任何一条改动能解释它消失（229 那条只对第三方输入法起作用，作者机器上没有）。
    差别更可能在于**他之前用的是打包的 v0.0.3**，那一版既没有 ③ 的 mouseup 补丁、
    也没有这一轮的改动。所以这条按**「未复现，观察中」**记，不按「已修」记；
    记录仪与上面那份排除清单都留着，下次再撞见直接取证。
  - 所以 `src/ui/ime-trace.ts` 留着（默认不装，只读一次 `localStorage`）：
    开关 `localStorage.setItem("dawn.debug.ime","1")` + 刷新，复现后 `dawnImeTrace()` 取记录。
    **仍然没有照着猜去改界面代码**——这一条要么等那份记录，要么等「作者用的是哪个输入法」
    这条事实（系统拼音 / 搜狗 / 微信 / 豆包，四者的事件序列完全不同）。

---

### 2026-09-07 — 照着文献里的图，在自己的数据上画一张

- **Type**: feat
- **Motivation**: 作者要的是*「看到别人的图 → 在我的数据上画一张同类的」*，
  并问识图之后能不能在 `MLAI-science` 里做相似检索。
- **What**（分支 `paper-figure`，规格 `specs/2026-09-07-照着文献画图-design.md`，定案 F1–F8）：
  **这一轮几乎没写生产代码，因为四段链里已经通了三段**——附件（粘贴/拖拽）、识图
  （多模态直接看 / `look_at_image`）、`run_code`。缺的只有一条 MCP 接线与一份走法。
  - `skills/figure-like-this/SKILL.md`：把图描述成**四行结构化**（图型/几何/标注/版式）→
    `recommend_figures`（要配方）+ `search_figures --mode semantic`（要参照）→ 读 `how_to` →
    `run_code` 在用户数据上画 → 落产物。
  - `docs/照着文献画图-怎么装.md`：那台 MCP 怎么配、库怎么建、搜不到时怎么分清四种原因。
  - `tests/skills/自带技能.test.ts`：**此前 `tests/skills/` 只验解析器，没有一条盯着技能内容**。
    新扫描验 frontmatter 齐全、`name` 与目录名一致、有「什么时候不要用」那一节。
- **Impact**: 配好那台 MCP 之后，「这种图我也想画」这句话有了一条确定的走法，
  背后是 1538 个跑过的案例，而不是模型的印象。
- **Verification**（**先量后写，六件事实里有两件当场改了设计**）：
  1. `recommend_figures` **自己做图型对齐**并告诉你哪个词匹上了哪个图型 →
     原设想的「先 `list_taxonomy` 拿词表」这一跳被删掉，降级为兜底。
  2. **1538 个案例里 1077 个是 draft（70%）**，`how_to` 带 `<!-- mlai:notes=draft -->` →
     「这是自动摘要不是配方」这句话是**常态**，不是边角，所以写成硬规矩。
  3. 拿「视觉模型会写出来的那种描述」跑 `figures --mode semantic`，**第一条命中就是被描述的那张图**。
  4. **用 DAWN 自己的 MCP 客户端真连了一次**：972 ms 起来、九个工具齐、
     真调 `recommend_figures` 拿回 `figure_types` 与两条非草稿的命中。
     顺带纠正技能里两个会出错的细节：**模型看到的工具名带服务器前缀**；
     **`notes` 成功时也有**，所以判据只能是 `figure_types` 空不空。
  5. 新扫描当场咬到一份真的缺（`office-docs` 没有「什么时候不要用」），已补一节实质内容；
     又故意把新技能的 `name` 改错一次，确认它会红。
  6. 单元 2768 全绿、`npm run typecheck` 干净。
- **还没验的（明说）**：**没有真跑过一次完整的「粘图 → 出图」**——那要作者拿一张真图和一份真数据，
  在旁边看着走一遍（计划 T4）。技能的第二版按那次的观察改，不按我的想象改。

---

### 2026-09-06 — 切了 v0.0.3（tag 已打，未推）

- **Type**: chore
- **Motivation**: 这一轮两件（应用内更新、输入法回车）都在 `main` 上了，作者要发一版。
- **What**: `npm run release:tag -- 0.0.3` —— `package.json` 0.0.2 → 0.0.3，提交 `81ee93b`，
  打上 `v0.0.3`。**没推**（推归作者，纪律照旧）。推上去之后 CI 按 tag 检出、四个平台打十个包、发正式 Release。
- **Impact**: **要说清楚一件容易想反的事**——`v0.0.2` 里**没有**应用内更新那套代码
  （它是 09-06 才写的）。所以装着 0.0.2 的人**不会**自己看到「有新版本 0.0.3」，
  他们仍然得自己去 Releases 页下载。**自更新这条路从 0.0.3 → 0.0.4 才真的开始起作用。**
  这也意味着 0.0.3 → 0.0.4 那一次是它的第一次真实检验（本地两个真包演练过，线上还没有）。
- **Verification**: 打 tag 前工作区干净；`release-tag.mjs` 自己挡了两件（脏工作区、tag 与
  `package.json` 版本不符）。发布结果要等推上去看 Actions 的 package 那条。

---

### 2026-09-06 — 输入法组词途中的回车，不再被当成「发送」

- **Type**: fix
- **Motivation**: 作者报的：在输入卡里打了 `woyaoyigewenjian`，候选词**还没上屏**就按回车，
  那一串拼音被当成话发给了模型。他同时指出「codex app 与 claude app 里没有这个问题」——
  确实没有，因为那类输入框都挡了 `isComposing`；我们只在笔记本的 cell 里挡了，输入卡上一处都没有。
- **What**:
  - 新增 `src/ui/ime.ts` 的 `在组词(e)`：`nativeEvent.isComposing || keyCode === 229`。
    第二条是给「被输入法吃掉的按键报成 229」那类平台留的，多它不花什么。
  - 八处按 Enter 做事的键盘处理器前面各加一句 `if (在组词(e)) return`：
    两张输入卡（含它们的 `@` 菜单与 `/` 菜单——那两处的回车同样不许抢）、
    「修改这段话」、重命名、命令面板、记忆新条、文件面板、网页地址栏、设置里的色值。
  - **配了扫描测试**（准入规则 2）：`tests/ui/design-contract.test.ts` 里新增一条，
    对每个键盘处理器做花括号配平取出函数体，**只要它认 `"Enter"` 就必须先问 `在组词`**。
    加上它当场报出了五处漏网的。
- **Impact**: 中文/日文/韩文输入法下，组词途中的回车回到它本来的含义（「就用这个候选词」）；
  候选上屏之后按回车照旧发送。**没有新增任何配置**。
- **Verification**:
  - **先量后改**：用 CDP 的 `Input.imeSetComposition` 在真产物上复现了一次，
    渲染进程收到的是 `keydown · key=Enter · code=Enter · keyCode=13 · isComposing=true`，
    而当时框被清空、`.conv-title` 出现、轮次数 2——**确实发出去了**。判据是量出来的，不是照记忆写的。
  - 那次探针改成了常驻用例 `e2e/ime.spec.ts` 两条：组词途中按回车**一个字都不许发**（拼音还留在框里）；
    候选上屏之后按回车**照旧发得出去**（修组词那件事时把正常发送一起挡住，是这类修法最常见的死法）。
  - 先红后绿：加守卫之前第一条红、第二条绿。
  - 单元 + e2e 全跑。

---

### 2026-09-06 — 应用内更新：点一下下完、点一下装上，API key 自己跟过去

- **Type**: feat
- **Motivation**: v0.0.2 发出去之后，应用里**没有任何一个地方提到过版本**（`src/` 里一处
  `app.getVersion()` 都没有）。装了 0.0.2 的人不会知道 0.0.3 存在；就算知道，换一版之后
  钥匙串不认上一版的密文，API key 与 SSH 口令要重填一次。两件加起来，每一份发出去的包都是死路一条。
  作者当天把要求提高了一档：*「照搬 codex app 以及 claude app —— 点击之后直接基于本机的系统版本，
  下载对应的版本，然后直接安装。并且 API-Key 应该是直接自动地替换为新的了。」*
- **What**（分支 `self-update`，规格 `specs/2026-09-06-应用内更新-design.md`，计划 `plans/2026-09-06-应用内更新.md`）：
  - **先量后写**。那两个 app 的顺滑是**签名买来的**：`electron-updater` 在 macOS 上走 Squirrel.Mac，
    要求新旧包同证书签名。我们不签名，于是先在真的 0.0.2 包上量了四件事——
    ①Releases API 匿名可用（60 次/小时）；②**node 的 `fetch` 下下来的包不带 `com.apple.quarantine`**；
    ③把解出来的 `.app` 放进一个 Applications 目录、走 LaunchServices 起来**零弹窗**；
    ④`app.getVersion()` 在打包版里读的是包内 `package.json`。②③成立才有这条路线。
  - 新增 `src/update/`：版本比较、按平台挑资源（十个资源名从真 Release 抄的）、发布源（真 GitHub 与假 feed
    **同一个解析器**）、24 小时节流、下载（进度 + 大小核对）、换包（备份→替换→失败滚回）、
    key 交接（AES-256-GCM，密钥从机器 id 派生）。
  - 协议 7.32：六个操作回**同一个信封**（完整状态 + 自动检查开关）；进度走事件通道的**第四种载荷**。
  - 界面：侧栏底部一行（常驻带文字）+ 设置「关于」一格（当前版本、手动检查、自动检查开关）。
  - 假发布源 `scripts/fake-release-feed.mjs`：`dev:mock` 与 e2e 共用一份（准入规则 1）。
- **Impact**: 装了旧版的人在应用里就看得见新版，点一下下完、点一下装上重启，**不用重填 key**。
  四个平台能到哪一步如实写在规格 U3 里：mac zip / win nsis / AppImage 全自动，
  **deb 与 win 便携版做不到**（要 root / 运行中的 exe 被锁），退回下载页并说明原因。
  不自动重启（这台机器上随时有内核跑着）。**Windows 与 Linux 的换包这台机器上验不了，明确标注未验。**
- **Verification**（准入规则 3；**四个真机验证各抓到一个只有真机看得见的洞**）：
  1. `probe` 真产物 + 假 feed → 抓到 **`app.getVersion()` 在开发模式下回的是 Electron 的 43.3.0**，
     于是线上 9.9.9 被判成更旧、整条功能安静地不出现。改为构建时钉进版本号。
  2. `npm run rehearse:handoff`（打包版 + 真 key）→ 交接那条路全过。
  3. `npm run rehearse:real-update`（**两个真包 0.0.90 → 0.0.91**）→ 头两次都红：
     第一次是演练脚本等 15 秒就 `app.close()`，把解包的 `ditto` 一起杀了；
     第二次红出了**真 bug：服务层拼参数时漏了 `应用路径`**，而它当时是可选的——
     十个单测全绿（每个用例自己传了它），真机上表现为「点了重启并更新什么都没发生」。
     修法不是补一行，是把它变成**判别式里的必填**（`换包参数`），让编译器管住这一类。
     同一轮补上：换包失败**要进 `startup.log`**（此前只推给界面，而 `failed` 不在侧栏出现，
     屏幕上什么都不剩）；`failed` 带 `失败于`，人自己按过的那两步失败要在侧栏看得见；
     换包期间按钮变「正在装…」并禁用（解 223 MB 要十几秒，再点一次会换两遍包）。
  4. 第三次全过：**0.0.90 装上 → 填真 key → 发一句 → 点更新 → 盘上真的变成 0.0.91、
     `.old-0.0.90` 留着 → 起来不重填就能再发一句真的。**
  - 单元 2763 全绿、`npm run typecheck` 干净、`e2e/update.spec.ts` 2 条 + 十张视觉基线一张没动。
  - 设计契约在实现途中拦下四件：裸 `<button>`、emoji 当图标、英文按钮互为子串、类名没配样式。
  - **一个已知的间歇红**（与本轮无关）：`tests/kernel/variables.integration.test.ts` 的 R 那条
    在全量跑时超时 90 秒，单独跑 1.6 秒通过——形状与 CI 的 Linux 挂死是同一类（资源争用），记在这里备查。
...
- **Type**: feat
- **Motivation**: 作者 09-04 把项目的意义收敛成一句话——**「活的内核」：数据读一次，之后所有追问都是增量的**，对标的是 RStudio / Jupyter 的工作状态，而不是路线图的阶段 ③（S21 Run 对比与 S22 审计当天被否决，理由与 S18 同一条：有 git + 代码 + 环境快照就够了）。集群上有两件事会把「活」打回原形：内核被 OOM 或作业系统杀掉，本地要等**5 分钟**静默兜底才知道（前一轮定案 10 的知情欠账）；掉一次线（合盖、VPN 抖），DAWN 直接判「内核死了」，于是读了三分钟的 `df` 要重读一遍——而那个进程其实还在服务器上活着。规格 `specs/2026-09-04-远端内核猝死与接回-design.md`（改写前一轮的定案 3 / 9 / 10），计划 `plans/2026-09-04-远端内核猝死与接回.md`（10 个任务）。**作者当场定的一条**：只接**意外掉线**；他自己按「断开」或关会话，照旧把服务器上的内核停干净——「意外才接，主动就是不要了」。
- **What**:
  - **心跳当报警，SSH 当结论**（`src/kernel/heartbeat.ts`，新）：五条隧道里 `hb_port` 一直转发着却从没打开过（enchannel 只开 shell/control/stdin/iopub），现在在它上面开一只 zmq REQ，空闲 10 秒、忙时 60 秒 ping 一次。**没回音不等于死**：ipykernel 的心跳在独立线程里忙时也答，而 IRkernel 单线程，长计算期间根本不答——所以沉默只触发一次 `kill -0` 确认，`DAWNALIVE=0` 才判死。判死后关通道、关隧道、删远端文件，转录说「内核在 `<服务器>` 上没了（可能是内存耗尽或被集群杀掉）」。
  - **5 分钟静默兜底改成先确认**（`src/kernel/挂载.ts`）：远端到点先问进程在不在，在就重新武装、只出声一次「进程还在，继续等」，不再把一段正在跑的长分析判死。顺带修掉「静默之后内核永远停在 busy」。
  - **掉线 = 分离，不是死**（`src/runtime/kernel.ts`）：链路一断就**同步**把这台记进 `分离的`（连接信息、pid、远端文件、语言、解释器、掉线时在飞），只收本地这半；新状态 `detached` 一路到笔记本胶囊（「等接回」，灰的——它没错，只是不在）与变量面板（「变量在服务器上没动」，**不画空列表**）。
  - **重连后认领**（定案 9/10）：`ready` → 扫残留（带一份「别动这几台」名单）→ 逐台 `kill -0` + 文件还在 → 用留下的连接信息重建五条隧道 → `kernel_info` 重握手（20 秒封顶）→ 同一个会话 id 回到 idle，转录「内核还活着，变量都在」；掉线时有段在跑的再补一句「那段可能已经跑完，输出没收到」。这三步是**同一条闸**，`run_code` 在它放行前不许起新内核。接不回来就停干净、说「没撑过这次断线」。
  - **扫残留改成逐文件杀**（`src/remote/kernel-launch.ts`）：旧的全局 `pkill` 分不出「上次留下的」与「等着接回的」。
  - 协议 7.31（`KernelState` 多 `detached`；测试专用 `fakeSshControl`，只在 `DAWN_FAKE_SSH=1` 时放行）；账本对掉线那段改口「与服务器断开，**这段的结果没收到**」——它多半跑完了，只是我们不在；假服务器加「掐线」（只断链路、不碰内核）与「杀内核」（SIGKILL、留文件）两个开关。
- **Impact**: 远端会话掉线后内核不再立刻判死，变量留在服务器上等接回；猝死从「5 分钟」变成十几秒。**只在 DAWN 这一次运行内接回，不进库**——关掉 DAWN 后服务器上那台由下次连上的扫残留清掉（如实记：DAWN 开着一直没连回去，那台会一直占着服务器内存）。本机内核的行为一个字节没改。协议是 minor，纯新增。
- **Verification**: 10 个任务逐个双审（规格 + 代码质量），审查抓到的都修了并各自补了判据，其中三条是真缺陷：①**分离时会往服务器发一条 `kill -KILL`**，正好杀掉打算接回的那台（当时只是碰巧因为执行器先被撤下才没生效，而测试把这条 kill 写成了断言）；②静默守卫里那条「确认死了」的分支不可达，真走到会让 cell 永远转；③重连时读「别动」名单早于掉线那条异步收摊填好它——现已改成掉线那一刻同步记下。**e2e 三条新用例是这一轮唯一证明「真的能用」的地方**（准入规则 3）：掐线 → 「等接回」→ 重连 → 手敲 `print(x + 1)` 得 **43**（那个 `x` 只可能来自活下来的那个进程）；背后杀掉内核 → 转录**30 秒内**出声（实测 19.5–19.9 秒，不是 5 分钟）；掐线期间内核死掉 → 重连说「没撑过这次断线」。实现者还做了证伪检验：在第一条里故意插一次杀内核，用例如期变红。`remote-kernel.spec.ts` 7 条全过（`--repeat-each=2` 无抖动），`test:e2e:only` 464 过（唯一一条红是 `files.spec.ts` 的老 flake，单跑即过）。**真集群上的合盖/掉线待作者验**；R 远端仍未真机验。

### 2026-09-05 — 假机器不许悄悄扫掉等着接回的内核：名单读不出来就落 127（分支 `kernel-reattach`，审查修复）

- **Type**: fix
- **Motivation**: 任务 8（`0aa5e3e`）让假机器的扫残留分支认「别动」名单，名单是用正则从脚本文本里抠出来的。审查用探针证明：脚本的 `case` 子句只差**一对引号**，正则就匹配不上，于是名单被读成空的——**被保护的 connection.json 被删、等着接回的内核被 SIGKILL，回执还是 `DAWNSWEPT=1` 加退出码 0**（探针实测 `{"out":"DAWNSWEPT=1\n","code":0}`、文件已经没了）。措辞归 `kernel-launch.ts` 管、以后还会改，这是准入规则 1 要挡的那一类：**假的悄悄说了一句会毁状态的谎**（`DAWNSWEPT=0` 写死那次藏住了一个真的内核泄漏，同一个坑）。
- **What**:
  - `src/remote/fake-ssh-kernel.ts`：命令里有 `case`（= 真脚本拼了名单）却读不出名单，就**一个都不动**、如实回 127 并在 stderr 里说清缘由——与合并探测脚本那条（认不得就落 127，不许只答半个键）同一个规矩。装机 id 读不出来时那句写死的 `DAWNSWEPT=0` 一并改成 127：那也是一个没人验过的固定答案，glob 改一个字就会让「每次连上先扫」在 mock 下整条空转还报「扫过了」。
  - `tests/remote/fake-ssh-kernel.test.ts`：新增「**正则钉在真脚本上**」——拿一个只记命令的 exec 去调真的 `扫残留` / `远端内核还在`，把它**原样**发出去的那条命令交给 `假内核命令`。以前的用例全是手打命令，证明的只是「假机器认得我手打的形状」。另加两条「读不出来就不扫」的判据（`case` 形状不对、装机 id 不对）。
  - 测试卫生：`重置假机器()` 顺手清 `掐们`（模块级的「还连着的链路」表漏给下一条用例，会让「掐断了几条」这个回执失去意义）；`掐断所有假连接()` 数**真掐断的**而不是名册长度；两个测试开关的**进程级作用域**写进导出函数的文档注释，与 `e2e/remote-kernel.spec.ts` 的 `假服务器开关` 互相指认；三条真 spawn ipykernel 的用例套上 `finally`——断言红了也把 detached 内核杀掉、文件删掉（否则每红一次就在 `$TMPDIR` 里留一台）。
- **Impact**: 只动假服务器与它自己的测试，生产路径一个字节没改。mock/e2e 下的行为变化只有一处：脚本与正则对不上时，扫残留**报错**而不是悄悄扫——按「失败必须出声」（规格 7.5），这正是要的。
- **Verification**: 先写探针用例看它红（量到上面那条 `DAWNSWEPT=1` + 文件被删），再修，再看它绿。`tests/remote` + `tests/electron` 241 过 2 跳过（带 `DAWN_FAKE_SSH_PYTHON=/usr/bin/python3`，真起内核的三条也跑了）；typecheck 干净；`npm run build` + `e2e/remote-kernel.spec.ts` **7 条全过**；跑完 `$TMPDIR` 里没有 `dawn-*.json` 残留。全量单测 2731 过 1 红——红的是 `tests/kernel/outputs.integration`（真本机内核 30s 等 idle），并行满载下的老毛病，单跑 5 条全过，与本次改动无关。
...
- **没做、记下**: 断线接回（pid 与文件路径已在内核状态里）；远端内核猝死（OOM、被集群杀）本地察觉不到，只有静默 5 分钟兜底（规格定案 10）；**R 远端路径没真机验过**（Spike F 只测了 Python，假服务器也只起 python）；Slurm 作业里起内核不做；`for p in $(which -a …)` 带空格的路径会被词分割。**真机验证（2026-09-04）**：作者在自己的集群上用 Python 跑过一次，原话「现在完全实现了我预期的功能」；R 远端仍未验。
...
- **Type**: fix
- **Motivation**: 作者看着打包版首启向导提的三条：「模型必须是一类，解释器必须是一类，用框架框起来（参考设置里的界面）」；「（可选，笔记本用；不配也能聊）太多余，直接写（可选）」；「手动填…没有显示完全，不要这么小家子气」。
- **What**: `.sw-section` 换上设置屏 `.set-rows` 同一副骨架（抬高底色 + accent 描边 + 大圆角 + 内距），模型 key 与解释器各成一框；`（可选，笔记本用；不配也能聊）` → `（可选）`；「手动填…」→「手动填写路径」（省略号是文案里写死的，不是截断）。英文表两条同步换（旧 msgid 删掉，孤儿扫描守着）。
- **Impact**: 只动首启向导一屏。向导不在视觉基线十张里，基线未动。
- **Verification**: interpreter-picker / setup-wizard / i18n / design-contract 4 文件 66 条全过；typecheck 干净；重新 `npm run pack` 后在作者机器上打开看过。
...
- **Type**: docs
- **Motivation**: README 停在 8 月 8 日，「当前状态」还写着「尚无代码」；仓库要公开了，别人第一眼看到的就是它。
- **What**: 头部一句话改成作者定的四个凸显点（开源、本地 AI 工作台、R/Python 数据科学、多智能体协作），刻意避开 wisp-science 的 Description 句式；新增「安装」（三平台 + 首启向导 + 外部依赖表）、「有什么」（13 项功能表）、「当前状态」（真实数字：7.7 万行、197 测试文件、107 e2e、485 条历史）、「从源码运行」、英文段；「四个支柱」「可追溯」「技术栈」「致谢」「许可」原文保留；文档表指向现存文件（旧表里 `2026-08-07-master-roadmap`、实体清单等已过时）。截图留了 TODO——e2e 基线是假模型 + 遮罩，不能当宣传图。
- **Impact**: 仅文档。
- **Verification**: 文档表里每个链接 `ls` 过存在；章节唯一（先前一次切片把旧「当前状态」「文档」带进来过，已清）。
...
- **Verification**: 单测（`migrate-userdata` 3 条）；在 mac 上交叉打 Windows x64 成功（安装器 + 便携版，解包目录里 win32-x64 的三个原生件都在；**未在真 Windows 上验**）；Linux 交叉打不成——node-pty 的 npm 包不带 linux 件，只能在 Linux 上打（写进文档）；`.deb` 要 `homepage`，先放占位。`npm run pack` + `npm run test:packaged` 在 mac arm64 上 5/5 通过（开库、发一句收假回复、`/` 菜单读到 22 份自带子 agent、终端里 `echo DAWN_PTY_OK` 回显、设置→内核列表）；`npm run dist:mac` 出 dmg。win / linux 的包**没在真机上验过**。
...
- **What**:
  ...
  - ⑥ 探针抓到的：解释器没配、内核起失败后面板冒出「内核已重起，上面 cell 里的变量已经不在了」——内核压根没起过。判据改为「有跑成过的 cell（ok 或有输出）」才算重起过。
...
- **首用回归 · 非 git 工作区**: 作者第一次真用就撞上临时会话满屏「本轮产出未知」。根因（在真账本里核过）：临时会话住在 `~/DAWN/scratch/<ts>/`，不是 git 仓库，`ProvenanceProbe.观察()` 的 `snapshot()` 抛 `NotAGitRepoError` → 探针不起 → 没有 `tool_files` → `files_written` / `files_created` 为 NULL。用户自己的非 git 分析目录同样中招。修法：新 `src/project/fs-facts.ts`（`fsSnapshot` 前后各扫一遍工作区，只记文件、不跟符号链接、跳过 `.git` / `.dawn` / `node_modules` / 虚拟环境；`fsDiff` 按 inode / mtime / size 对比；**超过 2 万条返回 undefined = 不知道**，不许截断后装作知道），`观察()` 在 `NotAGitRepoError` 时改走它，超限每个工作区只 `console.error` 一次；git 那条一字不动。两条路口径一致：缺省 = 不知道、空 = 确认没有；非 git 剔不掉作者手改的，`mayIncludeUserEdits` 如实为 true。两条旧单测「非 git 一律不报」按新口径改写（`tests/provenance-probe.test.ts`、`tests/runtime/外部工具溯源.test.ts`）。**e2e 顺手抓到第二处**：临时「项目」的 workspace 是 scratch 根，会话住在 `根/<ts>-xxxx/`，产物预览按 projectId 读差一层——坞里说「找不到」。`readFile` 加 `sessionId` 寻址（协议 7.24 → 7.25，纯新增；三选一；相对会话自己的工作区，与 `listArtifacts` 同一个根；远端会话仍按 connectionId），`App.tsx` 的 `读产物` 有会话就按会话读。spec §0 记首用回归，§2 改非 git 口径，§9 去掉那条。验证：新 `tests/project/fs-facts.test.ts` 5 条（跳过目录与符号链接、上限、`/` 分隔、diff 四态、仅 mtime 变）、探针 4 条（bash 写图 + 改旧文件、`beginExternal`、声明路径、超限只出一次声）；协议 2 条（三选一、版本 7.25）、后端 1 条（按会话读、没有这段会话出声）；e2e `artifacts.spec.ts` 加不 `gitInit` 的临时会话 describe（作者的原话场景：`GENERATED · 1` → chip 进坞预览，且没有「产出未知」）。单测 184 文件 2330 全绿；`test:e2e:only` artifacts + files 18/18；视觉基线 10/10 不用重存；typecheck、build 干净。
...
- **Type**: fix
- **Motivation**: 承接全库审查，清 D 域（微信/飞书通道）。先做一次"对代码核实真实状态"的甄别：D1/D2/D4/D5/D7/D9/D11/D12 早批（`8d3ea14`/`b072426`）已修、只是台账表列没同步；其余逐条修完。
- **What**: **D3** 微信 `startLogin` 的 `this.login` 在 `await fetchQrCode` 之后才赋值，并发第二次调用越过守卫起双份扫码流——改为进门同步占位（与飞书侧一致），取码失败清占位。**D6** 飞书"处理中"从单槽 `处理中消息` 改成 `处理中集`：两条正经问题几乎同时在飞时，单槽被后者覆盖导致前者 OnIt 撤不掉、后者被提前 DONE；回合结束时把集合里所有在飞消息一并收尾。**D8** 飞书 `sdk()` 每调一次都新建实例，`reactionIds` Map 每次都空 → 线上 OnIt 一次都撤不掉——backend 里记忆化成单实例。**D10** 飞书消息在写会话前就记入"已见"，写失败（瞬时）后被永久去重丢掉（at-most-once）——写失败时 `撤已见` 让 WS 重投可重试（at-least-once），保留写前记以挡在飞重投。**D13** 微信/飞书切段在 UTF-16 代理对中间硬切、把 emoji 劈成两个非法码点——切点前一位是高代理就回退一位。**D14** 飞书 `切段9000` 口径与微信 `切段` 对齐（换行点落在前半段就硬切，不吐零碎小尾巴）。**D15** 飞书 WSClient 的状态回调加 `stopped` 守卫，停掉的旧连接迟到的 `closed` 不再盖掉新连接的 ready。**D16** 微信图片下载失败时 `return` 把同条消息的文字也丢了——有文字就出声后继续处理文字。**D17** 微信 `unbind` 补清 `输入票`/`报过的工具`（飞书 unbind 同步补清 `已见`/`处理中集`/`待答`），换账号不复用旧 typing 票。
- **Impact**: 无破坏性 API 变更；飞书 `处理中消息:string` → `处理中集:Set<string>`（内部字段）。D 域 D1-D17 全部修完；D8/D15 属 real SDK 路径（fake 验不出），由逻辑+typecheck 保障。
- **Verification**: 单测 2244（一条 R 内核集成测试为环境相关 flaky，复跑 10/10 绿；与本改动无关）；新增回归：D13 emoji 不劈、D14 无小尾巴、D6 两条在飞都 DONE；typecheck 干净。累计约 83 条已修配回归测试。
...
- **Type**: feat
- **Motivation**: 作者两条：①「上传文件之后按钮该变黑，现在没有」（此前只有图片有 chips 依据）；②「上传文件按理和 @ 是一个样子」——此前插的是裸路径文本，不进引用栏。
- **What**: AttachButton 的文件 / 数据两项改插 `@相对路径` 令牌（同一份 `引用令牌` 语法：引用栏、点开、摘除、发送按路径+类型，全与手敲一致；带空白的路径进不了令牌，退回裸路径）；新 `附过文件` / `空态附过` 状态：插过就算「带上了」（按钮转黑），发送归零。
- **Impact**: 上传文件与 `@` 合流；附栏 ready 信号补全。
- **Verification**: composer-ready 4（含「非图片文件上传转黑、发送复原」）、attach、at-file 共 28 绿；视觉基线 10/10。

### 2026-08-25 — 上传文件换实心字形：实心灰起步，带上东西转实心黑

- **Type**: style
- **Motivation**: 作者：上传文件本身也该是实心灰，上传后变实心黑。原字形是描边圆圈 ＋，看着空心。
- **What**: 新 `实心圆加号图标`（实心圆、＋按 evenodd 镂空），`AttachButton` 触发键换用；灰 / 黑仍只靠上一条的 `data-ready` 颜色规则。描边版 `圆加号图标` 保留给别处。
- **Impact**: 附栏一颗图标；视觉基线重存。
- **Verification**: composer-ready / attach 22 绿；基线重存后连验两轮 10/10。
...
- **Type**: fix
- **Motivation**: 作者逐条：「-」太小改成点（照他给的截图，在跑的点是绿的）；title 换行之后才是 token 消耗；导出文件默认「会话标题 + 时间戳.md」；「会话」收纳折叠 / 展开是两个不同的图标。
- **What**: `.sess .name::before` 改成 6px 圆点、`li[data-running]` 时成功色；对话头 `.conv-head-text` 把标题与 `SessionUsage` 叠成两行，「导出对话」仍在右上；`导出文件名(title, id, when)` → `标题 20260823-090507.md`（本地时间到秒，导两次不互相盖；落在设置里的下载目录）；`对话描边图标` 改成与实心同一个气泡轮廓。
- **Verification**: 探针截图四张；`export.test.ts` 改了期望；相关 e2e 50 绿；视觉基线对话两张重存连验两遍。

### 2026-08-23 — fix-dug-0823 第二批：时钟真实心、会话行左挪 15、任务行也带「-」、项目文件夹各自展开

- **Type**: fix
- **Motivation**: 作者逐条指出：「最近」收起仍是空心（`时钟图标` 本身画的是个环）；「-」+ 标题要再往左 15；项目下没开口的任务行没有「-」；项目文件夹只能开一个。
- **What**: `时钟图标` 改成填满的圆盘（针用 evenodd 掏）；四处会话列表的缩进 38 → 23（文字左缘 62 → 47）；`.task-row > .name::before` 也画「-」；项目展开状态从一个字符串改成按路径记的 `Map`（人点过的按人的，没点过的装着当前会话就自动开）。
- **Impact**: 悬停卡与已归档屏上的小时钟也跟着实心。
- **Verification**: 探针截图（最近收起 / 展开、两个文件夹同时开、任务行带「-」）；侧栏相关 e2e 51 绿；视觉基线对话两张重存连验两遍。

### 2026-08-23 — fix-dug-0823 第一批：会话行前的「-」、固定入口描边、「最近」收起实心、设置图标实心、自带的技能 / 子 agent 能停用

- **Type**: fix
- **Motivation**: 作者列的五条。
- **What**: ① 会话行行首一个「-」（`.sess .name::before` 画，不进文本——读屏与判据都不碰它）；② 固定入口（＋ / 远端服务器 / 定时 / 已归档）换描边：新画 `加号描边图标` / `时钟描边图标` / `归档描边图标`，远端用现成的 `服务器描边图标`；③「最近」与项目 / 服务器 / 会话同一条：收起实心、展开描边；④ 设置里查下来只有「远程助理」那颗手机是空心的，改成实心（evenodd 掏 home 键）；⑤ 自带的技能 / 子 agent 能停用：自带文件在应用包里只读，档位落在设置库 `skill.mode.<名>` / `subagent.off.<名>`，`loadSubagentsFrom` 多一个 `自带停用`、运行时 `skillsOverride` 先问设置再看文件、设置屏的列表与 `setSkillInvocation` / `setSubagentEnabled` 对自带的走同一把键；菜单里自带的有开关、没有「删除」。
- **Impact**: 停用自带的从新会话起生效（与你写的那些一样）；`/` 菜单里停用的不列。
- **Verification**: `tests/subagent/roster.test.ts` 加 `自带停用`；`skill-ops` / `subagent-ops` 两条「自带的拒改」改成「落在设置里、文件不动、删仍拒」；e2e `skills.spec` 自带的换档并切走再回来留得住、`subagent-roster.spec` 自带的停用后 `/` 菜单里没了再启用；单元 2122 绿；e2e 全量 408 + 14 红——8 条是「-」进了文本（改成伪元素后绿），6 张视觉基线重存（diff 只有侧栏三颗图标与会话行；命令面板两张照旧拿 actual 救回）连验两遍绿。
...
- **Type**: feat
- **Motivation**: 作者拿着 dsh-agent-teams 面板的截图：*「我也要有这个类似的 team 页面」*。
- **What**: 新建 `src/ui/team-dag.ts`（派生态、依赖深度分列布局、上下游链、短标题、头像色号 / 首字——纯函数）；`src/ui/team-panel.tsx` 重写成六段：标题行（成员 / 完成 / 消息）、队长卡（已派发几项给几人、在跑 / 已收齐）、总进度（一任务一段 + 图例 + 一句总结）、成员树（竖线分叉、首字头像、人设、右边「已交付 n/m」、状态句、「队长派发」芯片；默认收起，点开仍是原来那份任务 + 消息 + 输出）、队长自己的 / 共享池、任务依赖小 DAG（悬停高亮链路、点击固定、Esc 松开、选中任务详情行）。样式整块重写；在跑那一段用条纹与已交付区分。e2e `team.spec.ts` 改成先点开成员再看任务。
- **Impact**: 只动坞里那一格；协议、磁盘真相没变。
- **Verification**: `tests/ui/team-dag.test.ts` 7 条；`tests/ui` 506 绿；单元 2118 绿；`e2e/team.spec.ts` 3 绿；探针造了 5 成员 6 任务的团队截图中途与结束两帧对照作者的图看过；全量 e2e 422 绿（十张视觉基线没动，团队格不在基线那几屏里）。
...
- **Type**: feat
- **Motivation**: 作者：「我要边缘距离更左侧边栏更远啊……我想和项目收纳、IP 地址保持距离，然后分出层次」。我前一版把会话缩到了父级左边（40 < 机器名 45），反了。
- **What**: 量出来机器名文字 45、项目名（含文件夹图标）63；四处会话列表（最近 / 项目底下 / 机器底下 / 散的）左内距统一 38，文字左缘 62（先做成 72，作者说太远、往左挪 10）——仍比 IP 文字深 17。
- **Impact**: `remote-connections.spec` 那条「服务器行与会话行同一条线」的左缘判据改成与固定入口行比（服务器行是面板里的顶层项），右缘与状态词仍与会话行同线。
- **Verification**: `sidebar-columns.spec` 第四条线（所有会话标题只有一条左缘）；侧栏相关 spec 绿；视觉重存后连验两遍。

### 2026-08-22 — 侧栏默认宽 264 → 224：数字列 188、三角贴数字 10、三角贴右缘 10

- **Type**: feat
- **Motivation**: 作者定的三个数：数字在 188 那条线上、三角离数字 10、侧栏右边离三角 10。三者同时成立只有一解：侧栏 = 188 + 10 + 16 + 10 = 224（此前照 WorkBuddy 的 264，右边一直空一截，作者给图两次）。
- **What**: `--dawn-sidebar-w` / `--dawn-sidebar-content-w` / `SIDEBAR_DEFAULT` → 224；右边各列全部改成从右缘倒着算（拖宽拖窄关系不变）：⋯ / ＋ 右缘 = 右缘 − 8；数字列（时间 / 各计数 / 多选 / 服务器状态词）= 右缘 − 36；会话行与服务器行右内距、右外距归零；`--dawn-row-tail` 36。
- **Impact**: 拖过侧栏的机器会记着旧宽度（`dawn.global.sidebar-width`），那台要把侧栏拖回或清掉那个键才是 224；`sidebar-size.spec` 往左拖的量从 120 改 80（224 + 60 − 120 会撞到下限 200）。视觉基线十张全部重存。
- **Verification**: `sidebar-columns.spec` 量到 数字列 188 · 三角 214 · 侧栏 224；remote / sidebar 各 spec 绿；全量见提交。

### 2026-08-22 — 「远端服务器」那一行：数字到三角 10px、三角到侧栏右边 10px

- **Type**: fix
- **Motivation**: 作者给的两个数。
- **What**: `.remote-head` 右内距 0、宽 `calc(100% − 12 − 11)`、右外距 11（三角盒 1em 带半像素零头，量出来 11 才是 10）；数字右留 3（行 gap 8 + 3 = 10）。这一行的数因此不在 188 那条线上——它跟着三角走，两个数定下来就只能这样。
- **Verification**: `sidebar-columns.spec` 改成直接断言这两个数 = 10 / 10；`remote-connections`、视觉绿。

### 2026-08-22 — 服务器收纳里的机器名贴着三角，不再漂到中间

- **Type**: fix
- **Motivation**: 作者给图：三角与机器名之间空出一大截。根因：前一条修截断时把 `.name` 改成 `flex: 1`，而它坐在按钮里（按钮默认 `text-align: center`），格子比文字宽时文字就漂到中间。我的 e2e 用的是超长名字、刚好截断，格子没比文字宽，所以没露出来。
- **What**: `.side-subhead .name { text-align: left }`。
- **Verification**: `remote-connections.spec` 新增「短名字贴着三角」一条（去掉修法确实红），原截断那条也加了同一判据；视觉、量线用例绿。

### 2026-08-22 — 「远端服务器」的三角落到收尾那一列（上一条改错了，已回退）

- **Type**: fix
- **Motivation**: 作者再给图并指出我改错了：要减的是**三角到侧栏右边**那截空，不是三角到数字的。上一条「三列整体右移 16px」已 `git revert`。
- **What**: 三角右缘落到 240——与会话行的「⋯」、项目行的「＋」同一条收尾线；数字仍在 188。算式写在样式表里（行自带 8px gap）。
- **Verification**: `sidebar-columns.spec` 三条线各一条；`remote-connections.spec` 绿；视觉 10 张重存后连验两遍；截图确认。
...
- **Type**: fix
- **Motivation**: 作者给图：「折叠和个数有很大的 gap，必须缩小点」。缝来自计数要落在 188 那条线上、三角钉在行尾。
- **What**: `.remote-head .side-count` 右留 4px，三角紧跟；行尾的空改留在三角右边（26px），计数那条线不动。
- **Verification**: `sidebar-columns.spec`（计数右缘仍 188）、`remote-connections.spec`、视觉 10 张绿；截图确认。

### 2026-08-22 — 侧栏分区标题：「多选」并入时间那一列，分区计数紧跟标题、自成一列靠左

- **Type**: feat
- **Motivation**: 作者：「多选的地方也要和其他会话的时间保持一列」「最近 / 项目 / 会话 / 服务器里面的会话个数也要保持一列」——先说靠右 18px，随即更正为「靠左边，而不是靠右边」。
- **What**: 分区标题文字包成 `.side-section-title`，占固定 3em（「服务器」最宽），计数 `.side-section-count` 紧跟其后——于是四个分区的数落在同一条左缘上；`.side-bulk` 右留 52 使其右缘落在 188（= 会话时间 = 固定入口计数）。中间那版「定宽 28px 靠右 18px」只活了一个提交。
- **Impact**: `sidebar-columns.spec` 量三条线：收尾动作列（右缘）、数字列含多选（右缘）、分区计数列（左缘）。
- **Verification**: 量出来 多选 188 · 分区计数左缘 119（三个分区一致）；视觉基线重存；全量 e2e 见提交。

### 2026-08-22 — 「增强」改叫「优化输入」、去掉常驻的「先写点什么」；终端也进卡底附栏

- **Type**: feat
- **Motivation**: 作者：「增强后面的先写点什么，其实感觉有点儿不好看」「终端我也建议搞下来」。
- **What**: `EnhanceControl` 文案「增强」→「优化输入」；空草稿时按钮灰着，理由进 `aria-label`（「先写点什么再优化」——不能是「优化输入」的超串，按钮子串那条扫描会拦），旁边那句不再常驻，`.enhance-note` 撤。终端开关从卡内那一行挪进附栏：上传 · 目录 · 终端 · 优化输入 | 权限；卡内只剩模型与发送。
- **Impact**: 此前「灰要说为什么，这句常驻在旁边」那条做法在这里让位给作者的观感；理由仍在，只是不占版面。
  另：作者给图指出服务器收纳里那台机器标题行的计数比别的数靠右 27px——`.side-subhead .side-count` 补回去；`sidebar-columns.spec` 扩成含服务器收纳与已归档的全量量线。
- **Verification**: `enhance.spec`、`attach.spec` 那条几何用例按新形状改写；视觉基线重存；全量 e2e 见提交。

### 2026-08-22 — 输入卡学 WorkBuddy：卡内一行 + 卡底附栏；侧栏收尾那一列一条右缘；切会话时快照被作废的根因

- **Type**: feat + fix
- **Motivation**: 作者看完第二档：「对话框的地方实在是太多了」，拿 WorkBuddy 的图点名——把上传 / 选目录 / 增强放到对话框整体的下面、用不同颜色区分；另报项目行的 ＋ 与「多选」不在一列。
- **What**:
  - 输入卡拆成 `.composer-card` = `.composer-box`（写、终端、模型、发送）+ `.composer-footer`（上传 · 目录 · 增强 · 权限，侧栏那档灰）。这条附栏 2026-08-12 抄过、8-16 撤了（那时只有一颗 chip），现在四样、有理由回来。圆角阴影聚焦环挂卡上；**卡不许 `overflow: hidden`**——菜单往上弹，剪了就点不到（e2e 抓的）。对话与空态两张卡同一套。
  - 侧栏：项目行与会话行右内距 8 → 24，「多选」「⋯」「＋/删」三个右缘（240 / 256 / 264）并成一条；数字那一列的 margin 跟着 36 → 52、14 → 30，仍与会话时间同线。新增 `e2e/sidebar-columns.spec.ts` 量这两条线。
  - **修**：`loadRunDetail` 借用 `guard()` 的全局世代，切会话时恰有一条 Run 详情在飞，整份会话快照（含会话开关）就被判成过时丢掉；症状「第一次点开没有权限那颗，切走再回来就有」。现在它用自己的世代。
- **Impact**: e2e 里 `.composer-controls` 选择器改指 `.composer-card`（两行都在里面，意图不变）；`attach.spec` 的「一行六格」改写成「卡内一行 + 附栏一行 + 两种底色」。视觉基线重存。
- **Verification**: `codex-polish-2.spec` 加一条回归（先有别的会话再点开新的，会话设置第一次就在）；vitest、全量 e2e、视觉见提交。
...
- **Type**: refactor
- **Motivation**: 作者两句话：*「概览应该是针对单一一个会话的，
  而不是全部的概览」*；*「设置永远都固定在下面，如果会话的内容太多了，
  就用上下滑轮去寻找」*。概览的七块面板里有三块是项目级的——
  同一屏在答两个问题。
- **What**:
  - `loadRuns` 带上 `sessionId`（协议与后端一直支持，界面从没传过）：
    成本与历史从此是这段会话的。**「状态」面板删了**（侧栏本来就在答它，
    两处答同一件事等于没有判据）；**「移除项目」移出概览**（项目级的
    危险动作不躺在会话概览里；项目行上那颗与它是同一个 `askDeleteProject`）。
    没选会话时概览只有一句指路的话，不摆一排空面板。
  - 侧栏：`.sidebar` 不再整体竖滚，会话那一片进 `.side-scroll` 自己滚，
    顶部动作与底部「设置」钉住。
- **Impact**: 纯界面；协议不变。项目级跨会话账本从此没有界面脸
  （数据仍在库里；将来阶段 ③ Run 对比是它该长的地方——作者知情）。
- **教训（当天踩的）**: 第一版把 `sessionId` 加进了「项目变了就装载」那个
  effect 的依赖——建任务的那一拍两个 atom 分两拍更新，effect 带着过期的
  projectId 重放 `loadSessions`，刚建的会话被旧项目的空列表冲掉，
  症状是**用户自己那一轮不渲染**（`task-workspace` 两条 e2e 当场红，
  全量跑出来才看见）。改为：那个 effect 只做项目级初始装载，
  会话作用域归 `refreshLedger`（只在有人看着账本时跑，依赖里本来就有
  `sessionId`）。
- **Verification**: `delete.spec` 的「移除项目」入口迁到项目行（同一个动作）；
  新 e2e：塞 30 段真会话把列表滚出溢出，量 `.side-scroll` 真在滚、
  「设置」的盒子仍在视口内；概览两张基线按纪律看过 diff（红的正是
  面板→空态那句）后重存并复验两遍；`sidebar-collapse` 那条的量具从
  `.sidebar` 搬到 `.side-scroll`（滚动条在哪量具在哪，守的东西没变）；
  1834 单元 / 363 e2e 全绿。
- **Commit**: `ba7ca33`
...
- **Type**: refactor
- **Motivation**: 作者两句话：*「在线视觉服务的内容，应该单独做一个卡片 box」*；
  *「左边侧边栏的项目概览……应该没有任何意义了」*，随后定了去向：
  *「换个入口，面板……审阅、文件、概览、网页」*。概览答的是
  「这个项目发生过什么」，看事实的时候人多半正在对话——坞正是
  「对话旁边看事实」的位置；原来那一整屏还会把对话顶掉。
- **What**:
  - 坞房客 `"overview"`：页签序 审阅 · 文件 · 概览 · 网页；七块面板
    （状态/成本/上下文/变量/环境/历史/移除项目 + 溯源）竖着摞（`.dock-overview`）。
  - **管子跟着搬**（2026-08-17 的教训原文就写在 `refreshLedger` 上）：
    变量/环境两个 effect 与 `在看账本` 的条件都改看「坞开着且在概览」。
  - 侧栏底部「项目概览」摘掉；`View` 联合去掉 `"panel"`（与 `"files"` 同一条理由）；
    命令面板那条改名「概览」、动作改为打开坞的那一格。
  - 视觉服务包进 `.vision-card`（带边框，照作者截图那张卡）。
  - 崩溃屏独占的 `.panels` 从三列网格收成一列。
- **Impact**: 纯界面搬家；协议、数据一字不动。删除确认里那句
  「记录留在项目概览」改说「坞的概览」。
- **Verification**: 判据翻面三处（侧栏不许再有那一行 / sidebar-states 那条 /
  app-mvp 迟到回调那条换「设置」演——概览不再顶掉对话，原断言对它不再成立）；
  受影响 e2e 全改走坞页签，42 条相关 e2e 绿；视觉基线「项目概览-*」删除、
  「概览-*」新增（截图人眼核对过形状），重存后复验绿；1832 单元全绿；
  全量 361 e2e 全绿。
- **Commit**: `b7fe50a`
...
- **Commit**: `17e5b47`（spec）+ `fe8ebce`

### 2026-08-20 — 坞拉宽之后的两栏换边：左树、右预览

- **Type**: refactor
- **Motivation**: 作者：*「图片展示和文件树换个位置，文件预览在右侧，
  文件树在左侧。」* 08-19 那版照 Codex 截图做成左预览右树，我的理由是
  「导航靠窗口边」——但在一个本来就贴着右边的坞里哪一栏都靠边，这条不构成
  理由；而他天天用的文件管理器全是左树右内容，手习惯往左找树。
- **What**: `styles.css` `.files-wide` 的 `grid-template-columns` / `areas` 对调，
  分界线从树的左边挪到右边；`App.tsx` / `files.tsx` / 设计文档里的描述同步。
  **判据翻面**：`e2e/files.spec.ts` 第 ③ 条从 `树.x > 预览.x` 改为 `<`，
  两边理由都写在注释与 `.files-wide` 里。
- **Impact**: 纯布局；窄坞的上下摞不受影响。
- **Verification**: `files.spec.ts` + 10 张视觉基线全绿（基线里没有一屏开着宽坞）。
- **Commit**: `70e6f7f`

### 2026-08-20 — 文件树：时间改成正规年月日时分；每一类文件一个图标

- **Type**: feat
- **Motivation**: 作者看过前一晚那版之后：*「时间戳不需要 刚刚 / 3m / 2h 这种形式，
  直接就是正规的年月日时间就可以了。此外，在文件预览的地方，能否增加图标，
  比如文件夹、文本、md 文件，各自都有各自的图标，这样子方便辨识。」*
- **What**:
  - `src/ui/format.ts` 新增 `年月日时分(iso)`：`YYYY-MM-DD HH:mm`，本地时区，
    不带秒，**不用 `toLocaleString`**（跟着系统语言走、不好测）。
    文件行改用它；`多久之前` 仍归会话行。`use现在` 不再往树里穿。
  - `src/ui/file-kind.ts` 新增：**只看名字**把一行判成
    `dir | markdown | text | table | image | code | shell | notebook | archive | pdf | other`。
    比 `分类预览` 粗——那一份决定「点开怎么显示」，这一份决定「一眼认出是哪类」，
    判的不是同一件事；且树里一层可能上千行，远端没内容可读。
  - `src/ui/icons.tsx` 新增九个同族图标：同一张带折角的纸，上面镂出不同记号
    （三行字 / M↓ / 四格 / 山与日 / 尖括号 / `>_` / 代码格+输出格 / 拉链 / P）。
    目录行用现有 `文件夹图标`，认不出的用现有 `文件图标`。
  - `src/ui/files.tsx` 目录行与文件行各配一颗 `.row-icon`；映射只有 `类型图标` 一张表。
- **Impact**: 纯界面；协议不变。
- **Verification**: `typecheck` 清洁；`tests/ui/file-kind.test.ts`（21 个后缀 + 多段后缀
  + 点开头）与 `format.test.ts` 两条；e2e `pick-workspace.spec.ts` 末条改为断言
  **今年的 `YYYY-MM-DD HH:mm`** 且 csv / md / 目录三行的图标 `d` 互不相同；
  一次性探针截了文件坞的图亲眼看过（图标在 16px 下分得开），探针已删；
  10 张视觉基线全绿。全量：1808 单元 / 361 e2e 全绿。
- **Commit**: `d347b14`

### 2026-08-19 — 文件树：文件行写着多久前改的；一颗「刷新」按钮

- **Type**: feat
- **Motivation**: 作者在服务器上跑数据分析，目录多半**不是 git 仓库**，
  所以「这一轮生成了什么」没有事实层可问。作者自己给出了够用的形状：
  *「我只需要在文件树里面看到生成了什么数据就好，有时间戳，我就知道哪个文件
  是新生成的了。」* 以及 *「可以给 DAWN 的文件里面增加一个刷新的按钮……
  多刷新其实就好了。」*——不做快照比对，不做自动侦测。
- **What**:
  - `src/remote/ssh.ts` `readdir` 多取 `attrs.mtime`（秒→毫秒 `mtimeMs`）。
    **此前后端给远端条目填的是 1970 占位**，注释写着「SFTP 不给修改时间」——
    错的，它就在旁边；又一次「东西到手了，最后一步没往下传」。
  - `src/workbench/backend.ts` 远端列目录 `modifiedAt` 改为真值。
  - `src/ui/files.tsx`：文件行右侧写 `多久之前(modifiedAt)`（与会话行同一套
    `刚刚 / 3m / 2h / 5d`），悬停是完整时间；目录行不写（目录 mtime 含糊，
    与不报大小同一口径）。`现在` 由树根 `use现在()` 读一次往下传，不每层起心跳。
  - 刷新是树头那一行的一颗图标按钮（`刷新图标`，读屏名「刷新当前文件夹」，
    照作者常用的 SFTP 客户端的样子），走的是已有的 `刷新令牌` 那条路
    （本地计数与外部令牌**相加**），所以展开状态不丢。
- **Impact**: 本地与远端文件树都多一列时间；远端条目的 `modifiedAt` 从占位
  变成真值（协议形状不变）。无破坏性变更。
- **Verification**: `typecheck` 清洁；`tests/remote/ssh.test.ts` 桩补了 `mtime`
  并断言毫秒换算；设计契约扫描当场抓了两条（`title=` 与 aria-label/文案子串），
  按它改成图标 + `aria-label`；新 e2e（`pick-workspace.spec.ts` 末条）：
  绕开界面写一个文件、**只按刷新不切窗口**，行出现且 `.file-when` 写着「刚刚」；
  10 张视觉基线全绿（没有一屏开着文件坞）；全量：1783 单元 / 361 e2e 全绿。
- **Commit**: `ff736f5`

### 2026-08-19 — MCP 接得了跑在别处的那种：streamable HTTP 与 SSE

- **Type**: feat
- **Commit**: `待回填`
- **Motivation**: 作者：*「我要对接的是我自己放在云服务器上的 MCP……
  新的 streamable HTTP（一个 `/mcp` 端点），但是我觉得新的，或者老的，
  还是要适配的。」*

  此前只做了 stdio（起一个本地进程走管道）。云上那台**没有本地进程可起**。
- **What**: `McpServer` 从一个对象变成**两种形态的联合**：
  | | 本机那种 | 跑在别处那种 |
  |---|---|---|
  | 判据 | `command` | **`url`** |
  | 传输 | stdio | `http`（streamable，新）/ `sse`（老那套） |
  | 密钥 | 环境变量名 | **请求头名** |
  | 有 stderr 吗 | 有（连不上时的唯一线索） | **没有**，只有状态码 |

  - **判据是有没有 `url`，不是有没有 `type`**：`type` 有默认值（`http`），
    而 `url` 是远端那条唯一不可省的东西。
  - **两种都收**（作者要的）。只收新的等于把存量文档里那一半挡在外面。
  - **密钥纪律一字不改**：配置里**只留名字**，值走钥匙串。
    `{"headers": {"Authorization": "Bearer …"}}` 贴进来，
    那半句令牌**不会出现在配置文件里**（有一条判据专门盯这个字符串不落盘）。
  - 界面上那一屏给了**两个例子**——只给本机那个的话，人会照着它改，
    而远端那种连 `command` 都没有，**一个例子只教得会它自己那一种**。
- **类型系统把每一处「假设它是本机那种」都标了出来**——`cwd` / `env` /
  `command` 共七处。这正是联合类型该干的事：**改成 `any` 混过去的话，
  那七处会在运行时一个一个炸，而且都在真连服务器的时候。**
- **假服务器跟着长了一档**（准入规则 ①）。**没有另写一份 HTTP 的**：
  同一份 `scripts/mcp-test-server.mjs`，给它 `DAWN_MCP_HTTP_PORT` 就起
  streamable HTTP。两份的工具集迟早各自漂移，那时「本机那条能用」
  就不再说明「远端那条也能用」。
- **写完当场撞到两个坑，都留在注释里**：
  - **无状态的 streamable HTTP 不能复用一份 transport**：`initialize` 能过、
    第二个请求就废（客户端报 `Error POSTing to endpoint:`，而单独 curl
    一次 `initialize` 却一切正常）。按 SDK 的无状态用法**每个请求新建一份**。
  - **HTTP 头只装得下 Latin-1**：我拿中文当令牌试，`fetch` 抛的是
    `Cannot convert argument to a ByteString ... value of 25105`——
    **那句话对着一个刚填完令牌的人什么都没说**。加了一句人话，并配了判据。
    *（我两分钟内就踩到了，那就不是一个假想的用户错误。）*
- **Verification**: 1783 单元 / 360 e2e / 10 张视觉基线，0 失败。
  - 新增四条盯远端那条：连得上并列得出工具、调得动、
    **令牌真的跨过 HTTP 到了服务器那头**、少了密钥要点名。
    第三条是要害：**中间任何一环断掉，前两条照样绿**（假服务器不要求鉴权），
    而真实的云服务器会回 401——到那时才发现就太晚了。
  - **三条既有的「工具清单」断言**（两条单测 + 一条 e2e）从「一共这三个 /
    3 个工具」改成「这几个都在 / 有几个工具这件事在」：共用的假服务器
    加一个工具**不该**让它们红，**少一个必须**让它们红。
    上一版写死数字，等于把「假服务器有几个工具」这件无关的事
    也钉进了三条判据里。
  - 还有一条抓得漂亮：**英文模式下每一屏不许有汉字**。
    我在新加的那段示例 JSON 里写了 `https://你的域名/mcp`——
    那是 `<pre>` 里的字面量，不走 `t()`，于是英文界面上会露出汉字。
    改成 `your-server.example`。**示例代码块也是界面的一部分。**

### 2026-08-19 — 选完文件夹要**看得见**地进项目；文件树跟得上外面的改动

- **Type**: fix
- **Commit**: `待回填`
- **Motivation**: 作者用了上一轮那版之后报的两件：
  > *「我选择完文件夹之后，现在文件tree肯定是对应上了，但是我在挪动文件进去之后
  > 文件tree没有更新，同时应该立刻收纳入项目里面，现在也没有。」*
- **What**:
  - **① 在外面改了文件，切回窗口树就跟上。** 那个刷新令牌此前只在
    **我们自己动手**时才 +1（传完、上传、删除）；在 Finder 里挪进去，
    **没有任何东西会告诉界面**——屏幕上那棵树与磁盘上那个目录从此是两回事。
    现在窗口 `focus` 时重读一遍。
    - **不上文件监视器**：它要递归盯住整个工作区，而科研目录动辄几十万个文件
      （一个 `data/raw` 就够）。**盯住它们的收益只发生在你回到这个窗口的那一刻**，
      而「切回来」恰好就是那一刻，且它是免费的。
    - **顺带修掉一个没人报过的老毛病**：那个令牌此前拌在树根的 `key` 里，
      一变就**整棵树重挂、展开的目录全部塌回去**。传完一个文件就发生一次，
      只是上传不常做。现在它进 effect 的依赖，**逐层重读、展开状态不动**——
      也正因为改成了重读，「按 focus 刷新」才敢做（否则每次切回来树都收起来）。
  - **② 人刚亲手选的那个文件夹，一句话都没说也出现在「项目」收纳里。**
    只补刚选的那个，不是列出全部——列全部的话**每一个曾经点开过又没用的
    文件夹都会永远留在那儿**，而这一列是「我在哪几摊活儿之间来回」。
- **② 我第一版写宽了，十七条 e2e 当场拦下，含全部十张视觉基线。**
  我拿的判据是「**有一个活跃项目**」，而**应用一启动就有一个**（默认工作区）——
  于是那一列变成了**永远存在**，一个什么都还没做的新窗口上也挂着它。
  收窄成「**人刚刚亲手按下那颗按钮选的那个**」之后全绿。
  用例里留了一条专门守这个错：**没选过就不该出现，哪怕有活跃项目。**

  这一条值得记：**十张逐像素基线一起红，通常不是基线的问题，是改动的范围
  比想象的大**。当时我差一点就去看单张 diff 图了——而十张同时红这件事本身
  就是那条最有用的信息。
- **我上一轮把「进项目」理解错了，而且给这个错误配了一条判据。**
  他说「选完立刻进项目」，我理解成**内部状态**（文件树指过去就算数），
  还写了 `选完文件夹，侧栏一行都不多` 那一条，理由是「没有任务的项目不显示」。

  **一个只改变内部状态、屏幕上毫无反应的操作，与没生效分不开。**
  这一轮把那条判据翻了面（不是删掉）：**收纳要出现**是新加的，
  **而里面一段会话都没有**是它原本真正想保的东西——两头现在一起盯。
- **Verification**: 1772 单元 / 355 e2e / 10 张视觉基线。
  - `e2e/pick-workspace.spec.ts` 现在四条。新增那条**绕开界面直接往磁盘写**
    （`writeFileSync`）再派一个 `focus`——中间没有我们自己的上传/删除路径参与，
    模拟的就是「在 Finder 里挪进去」。**先撤掉改动验过它会红**。
  - 那条用例还顺手盯住「刷新不许把展开状态清零」：先展开一层再切回来。
  - 单测那一组「任务怎么分组」显式把 `activeProjectId` 摘掉了——
    **不摘的话它们会红在一个与自己无关的理由上**；新规则由单独一组盯。

### 2026-08-19 — 首页选完文件夹，立刻进项目、文件树跟着换

- **Type**: feat
- **Commit**: `待回填`
- **Motivation**: 作者先问的是一个设计问题：*「我在选择文件夹后，立刻进入项目，
  文件tree也转入呢？还是选择文件夹后在对话之后，才进入项目？」*
  量完之后他定：*「选完立刻进项目、文件树跟着换。如果不选文件夹，
  那么就按照会话进行处理。」*
- **动手前量到的三件，它们把选择空间压得很小**：
  1. **那个中间态是真的**：选完文件夹，路径只记在空态那一屏自己的 `useState` 里，
     **chip 上写着新目录、而文件树还指着旧地方**（树跟着 `projectId` 走）。
     屏幕上同时有两句话，说的是两个地方。
  2. **「树跟着走」与「进项目」拆不开**：本地 `listDirectory` **必须给 projectId**，
     路径相对工作区、绝对路径被守卫拒。想只切树不进项目，
     就得开一条读任意本地目录的口子——**那道守卫不值得为这件事拆掉**。
  3. **「立刻进项目」几乎没有代价**（我原以为会留垃圾，量完发现不会）：
     `projects.open()` **幂等**，同一文件夹永远命中同一条；
     而侧栏那一列**由任务分组而来**，**一个没有任务的项目在界面上根本不出现**。
- **What**: 新增协议操作 `openProject({workspace})`（76 个）。
  界面上选完文件夹 → 认领项目 → `setActiveProjectId` → 文件树跟着换。
  **不建任何会话**：「开口那一刻才建」那条决定管的是任务/会话，不是项目。
  认不下来**出声且不往下走**——静默吞掉的表现恰恰就是那个自相矛盾的中间态。
- **一条既有判据当场拦住了我，而且拦得对。** `openProject` **正是 T4（协议 5.0）
  摘掉的七个操作之一**，那条用例写着：*「删除也要有判据，否则下一个人
  『顺手』把它加回来时没有任何东西会响。」* **我就是那个人。**

  这一次是**有意翻面**，所以按规矩翻面而不是删判据——两边的理由都留在用例里：
  T4 摘它是因为**它当时是「开会话」那条路的一环**，而那条路只剩 `createTask`；
  现在加它是因为**文件树需要一个「把文件夹认成项目」的动作**，
  而它明确不建会话。名字沿用旧的（它确实就是那件事），
  但用例里加了一条**盯住它别再长回去**：请求参数只许有 `workspace`。
- **Verification**: 1770 单元 / 354 e2e / 10 张视觉基线。
  - `e2e/pick-workspace.spec.ts` 三条，**先把改动撤掉验过它会红**——
    红的样子正是作者报的那句「chip 变了、树没变」。
  - 另两条守的是反面：选完文件夹**侧栏一行都不多**（不许急着出现），
    以及**说了第一句话它才归到「项目」那一列**（也不许永远不出现）。

### 2026-08-19 — 挑模型这件事：前面写具体模型，每一条标出 API / CLI / ACP

- **Type**: feat
- **Commit**: `待回填`
- **Motivation**: 作者两句：
  > *「我要我选择的时候，直接是 Opus4.6 而不是 Default (recommended)。
  > 你可以在 Opus4.6 后面显示 Default (recommended)，显示 Most capable
  > for complex work，但是不要在选择的地方，选择 Default (recommended)。」*
  > *「我要你在模型选择的时候，标记出是 API 还是 CLI 还是 ACP。」*
- **What**:
  - **① 具体模型提到前面。** `拆模型名(name, description)`：说明里带 `·` 时，
    把 `·` 前面那一段提上来，原来的 `name` 与后半句退到第二行。
    **两台真适配器的约定不一样，这条规则是量出来的**：
    | | `name` | `description` |
    |---|---|---|
    | `codex-acp` 1.1.9 | `GPT-5.6-Sol (low)`（本来就是模型） | 散文，没有 `·` |
    | `claude-code-acp` 0.16.2 | `Default (recommended)`（角色名） | `Opus 4.6 · Most capable…` |
    照一台写就会把另一台切坏，所以两台的原话都进了单测。
  - **② 挑 LLM 的单子上每一条标出它是哪一路。** 此前**只有触发按钮上那一颗**——
    也就是说你看得见「现在这段是哪一路」，**看不见「要挑的这个是哪一路」**，
    而挑之前才是需要知道的时候：三条路在一件实事上不同
    （API 的权限门管得住、CLI 管不到也不问、ACP 管不到但会主动问）。
  - 顺带把 `AgentPill` 那个手抄的 kind 联合改成从 `SessionSummary` 推导——
    它漏了 `acp`（2026-08-16 就有了），**抄件与原件各自漂移的老毛病**。
- **我先拒绝过 ①，作者驳回了，而他是对的。** 我的理由是「解析别人的自由文本，
  人家改一次排版我们就开始胡说」。但**「Default (recommended)」在选择的地方
  等于什么都没说**，这个代价比那个风险大得多。
  让步的前提是把风险摁住：**这条解析只会退化，不会胡说**——
  没有 `·`、提出来的是空的、长得不像名字（>40 字）、或与 `name` 相同，
  一律原样。**最坏的结果是回到今天的样子。**
- **Verification**: 1770 单元 / 351 e2e / 10 张视觉基线。
  - `拆模型名` 六条单测，输入全是**从两台真适配器身上抄下来的原话**。
  - **假 agent 跟着改了形状**：它那一项原先的说明里没有 `·`，
    **那条提升规则就没有任何判据走得到它**。这是「假的不能在契约形状上
    比真的宽容」那条的第二次应用（今天第一次是 `mcpServers[].env`）。
  - **排版栽了两次，两次都是截图才看见的**（这个菜单不在十张逐像素基线里）：
    先是块级 flex 让勾跑到名字上面；改成 `inline-flex` 后一行字时对了，
    **而说明一旦换行又整块掉下去**。最后把整行改成 flex 才踏实。
    **「写了」与「看起来对」之间还隔着一次真的去看。**
...
- **Type**: chore
- **Commit**: `8c1e0f9`（收尾）+ 快进合并
- **Motivation**: 作者：*「如果是差不多了的话，我们就合并进入 main。」*
  这条分支从 2026-08-19 起，办了作者报的四件，外加过程中撞出来的四件。
- **What**:
  - **`--ff-only` 快进**。这个仓库 `git log --merges` 一直是 **0 条**，
    合之前是 0，合之后还是 0——**没有造一个合并节点**，
    10 个 commit 原样落到 main 上，每一条都带着自己的判据与理由。
  - 这一轮办掉的（前四件是作者报的）：
    1. **ACP 接得进来了**——代码 2026-08-16 就全做完了，缺的只是一个入口，
       于是它对使用者等于不存在；
    2. **点服务器里的旧对话连不上**——续接漏传远端执行器，且失败被空 `catch` 吞掉；
    3. **服务器收纳里的会话拖不动**——一道 2026-08-14 就已失效的过滤留了下来；
    4. **大图看不清**——预览挪到文件树旁边（**这一件我先做错了一版**）；
    5. 会话行那一格从 `alive` 换成「距离上一次对话多久」（作者要的，形状取自 Hermes）；
    6. 远端服务器那一行同理：连着 `alive`，连过又断了写时间；
    7. **坞能被拖出窗口外**（拖把手一直就能，不是新问题）；
    8. **一条在功能坏掉时才会通过的 e2e**（在干净的 main 上验过是它自己的问题）。
- **Impact**: main 第一次带上「时间取代状态字」这一套。
  **没有 push**——按约定由作者自己推。
- **Verification**: **在同一棵树上完整跑了一遍**：typecheck 干净 /
  1766 单元 / 348 e2e（含 10 张视觉基线）/ 0 失败。
- **这一轮值得记下来的三件**：
  - **作者两次把我拉回正道**，两次都是我多做了一层：
    「不能在主区放图，要放到文件区旁边」、「连接不上为什么不直接写 exited，
    在这里纠结什么呢」。**两次都是我用一个新概念去解决一个规则能解决的问题。**
  - **他问「计时做完了吗」时，我原本要说「做完了」**——去验之后发现两个洞
    （切走之后标记粘住、心跳从没跑过）。**「我写了」与「它跑过」之间那道缝，
    这一轮又量到一次。**
  - **十几条既有判据挂在被摘掉的东西上，一条条挪了锚而不是删掉。**
    其中「服务器行与会话行右缘对齐」那条是唯一有可能真坏的——量过，没坏。

### 2026-08-19 — 远端服务器那一行：连着写 `alive`，连过又断了写时间

- **Type**: feat
- **Commit**: `6fe1c55`
- **Motivation**: 作者：*「远端服务器也需要激活的时候 alive，非 alive 的话，
  就是显示时间。」* 同一轮里他还确认了**不按时间分组**——
  收纳仍然是「会话 / 服务器 / 项目」那三格。
- **What**:
  | 状态 | 那一格 |
  |---|---|
  | 连着 | `alive` |
  | 正在连 | `connecting` |
  | **连过、此刻没连着** | **一个时间**（上次连上是多久前） |
  | 从没连上过（含连不上） | `exited` |

  - 与会话那一列同一条理由：**`exited` 对任何一台没连着的机器都成立，
    所以它什么都没说**；而「上次用它是 2 天前 / 30 天前」是判断
    「这台还在不在、口令还有效吗」的依据。
  - **`connecting` 留着**：它是正在发生的事，且转瞬即逝。
    在人盯着看结果的那几秒里跟他说「上次是 3 天前」是答非所问。
  - **数据：schema v14 新加一列 `last_connected_at`**，在 `onState` 变成
    `ready` 的那一刻盖戳——那是所有连接路径的唯一必经之路
    （按钮连的、`造远端参数` 顺手连的、断线重连的）。
    盖在「按下连接按钮」那处的话，**按了没连上也会被记成连上过**。
  - **新列不必是空的：它能从已有的事实回填。** 一段会话跑在这台服务器上、
    且账本记着它在 T 时刻干了活，**那么 T 时刻我们必然连着这台机器**——
    这不是猜，是推论。不回填的话作者现有的每一台都会写 `exited`，
    而他明明连过。`WHERE last_connected_at IS NULL`：只补空的。
  - **两处心跳共用一个钩子**（`use现在`）：远端那一区长在 `App` 上、
    不在侧栏那棵树里。抄第二份的代价不是那几行，
    是**两份心跳迟早各自漂移**，那时人只会觉得「这个数有时候不准」。
- **我在这儿多想了一层，被作者当场按回去。** 我一度另造了一个「没连过」，
  理由是「刚加进来的显示『刚刚』会读成『刚刚连过』」。作者：
  *「我就纳闷了连接不上为什么不直接写 exited 呢？连接过了，然后断连了，
  直接写时间不就好了？在这里纠结什么呢？」*

  **他是对的**：`exited` 本来就是「没在跑」，而没连过正是没在跑。
  多一个词就多一套要解释的语义，而它没有多说任何事情——
  那条「读成刚刚连过」的担心，在「没有时间可写时就不写时间」这条规则下
  根本不会发生。**规则比例外少一条，语义就少一层。**
- **Impact**: 三种「没连着」比以前分得更开（以前全写 `exited`，
  只能靠原因行与点的颜色分）：没试过 = `exited` 无原因行；
  连不上 = `exited` **有**原因行；连过又断 = **一个时间**。
- **Verification**: 1766 单元 / 349 e2e / 10 张视觉基线。
  - `tests/store/runs.test.ts` 三条盯那句回填（从账本反推、
    已有准确值不被覆盖、**推不出来就留空不拿创建时刻顶上**）。
  - `remote-connections.spec.ts` 新增一条盯**整条路走到屏幕上**：
    连上 → 断开（写「刚刚」）→ 把时刻挪到三天前 → 重载 → 写 `3d`。
  - 四条既有判据挪了锚：三条状态词、一条切语言的
    （**那一条一个字没改，只是覆盖面小了一点**——时间本来就不跟语言变）。
