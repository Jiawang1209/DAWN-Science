...
describe("不变式：那一列只有一个位置", () => {
   ...
   * 破了它。表驱动跑遍「五个起点 × 八个动作」，比任何一条单独的场景用例
   * 都更值钱——它是会抓住*下一个*同类漏洞的那一条，不只是这一批。
   */
  it("五个起点、八个动作，做完之后窄栏与坞永远不同时开着", () => {
    const 重置到初始 = () => {
      localStorage.clear()
      $view.set("conversation")
      $rightDockOpen.set(false)
    ...
    const 动作表: ReadonlyArray<[string, () => void]> = [
      ...
      ["坞上位()", () => 坞上位()],
      /**
       * **第七、第八条，2026-09-17 补的**（Task 6 步骤 ⑩ 把 `打开设置整页` 从
       * `view.ts` 搬进了这个模块）。给不给 `section` 各来一条，与上面
       * `开设置栏` 那一对同一个形状——那两支在函数里走的是不同的分支。
       *
       * 搬家的全部理由就是它在旧地方绕开了这条不变式——**多一个导出的 mutator
       * 而不进这张表，这张表就开始名不副实**：它自称「会抓住*下一个*同类漏洞
       * 的那一条」，而下一个漏洞恰恰可能长在没被它扫到的那个函数上。
       */
      ["打开设置整页()", () => 打开设置整页()],
