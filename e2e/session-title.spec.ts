...
/**
 * **会话行右端那一格，回答的是「这段我搁下多久了」**（2026-08-19 换了主语）。
 *
 * 这条判据的问题一直没变——*「一眼看得出哪段是新的」*——换过两次答案：
 *
 * | 时间 | 那一格 | 换的理由 |
 * |---|---|---|
 * | 起初 | 双行：标题在上、`DeepSeek · 时刻` 在下 | — |
 * | 2026-08-12 | 单行右端写**创建时刻** `14:48` | WorkBuddy 实测 `240×31` 单行，双行 53px **高出七成**，一屏少放三分之一 |
 * | 2026-08-19 | **距离上一次对话多久**（`14h` / `2d`） | 作者要的（形状取自 Hermes）。`14:48` 要人自己拿今天几号去减；而量过作者的库，**创建时刻本身就是错的那个数**——一段 01:40 建的会话最后一次动作在 03:38 |
 *
 * **agent 名 2026-08-12 从这一行拿掉了**，不是丢了这条信息：
 * 每条回答上都记着是谁答的（`item.by`），composer 上还有一颗 pill。
 * 那时它必须在这里，是因为当时**只有这里说得出**；现在不是了。
 */
test("会话行右端那一格 —— 一眼看得出哪段是新的", async ({ dawn }) => {
  const { page } = dawn
  await 开一段临时会话(page)
  /**
   * 刚建出来就是「刚刚」。**不写 `0m`**——那读起来像「零分钟」，是个怪值。
   * 各档的边界由 `tests/ui/format.test.ts` 钉着，
   * 整条路（账本 → 协议 → 这一格）由 `e2e/session-when.spec.ts` 钉着。
   */
  await expect(page.locator(".session-list .sess-when").first()).toHaveText("刚刚")
})
