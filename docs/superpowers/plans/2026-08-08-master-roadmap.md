...
### 阶段 ①-C 外部 CLI 作为对话式 agent 🔄（2026-08-09 起）
...
| 批次 | 内容 | 状态 |
|---|---|---|
| 1 · 协议与配置 | C1 新增 `kind: cli`（协议升 2.3） | ⬜ |
| 2 · 驱动 | C2 claude（长驻进程）· C3 codex（一轮一进程 + thread_id） | ⬜ |
| 3 · 账本接线 | C4 CLI 事件 → Run | ⬜ |
| 4 · 形态归位 | C5 默认配置换血 + shell agent + 界面分支 | ⬜ |
| 5 · 验 | C6 e2e（用假 CLI，理由见计划 §5） | ⬜ |

**明确不做**：headless 下的权限/审批（Spike G 没验，与阶段 ④ 授权门同源，需单独 spike）。
