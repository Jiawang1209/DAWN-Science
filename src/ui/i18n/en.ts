...
export const EN: Readonly<Record<string, string>> = {
  ...
  "覆盖会用新文件替换旧的。不覆盖就只导没撞名的那些。": "Overwriting replaces the old file with the new one. Otherwise only the ones without a name clash are imported.",
  "这里没有可导的定义": "Nothing importable here",
  "一个 .md 一个子 agent：frontmatter 写 name / description（可选 tools / model / group），正文就是它的人设。": "One .md per subagent: the frontmatter holds name / description (optionally tools / model / group); the body is its persona.",
  ...
  "从别处复制来的一段话里有 @alice、甚至有真实存在的 @路径，都不开菜单、不发给模型。关掉则一视同仁。": "Text copied from elsewhere — @alice, even a real @path — opens no menu and is not sent to the model. Turn off to treat pasted text like typed text.",
  "全局过滤规则": "Global filter rules",
  "@ 菜单里不列这些文件名。只看文件名，不看父目录。所有工作区共用。": "The @ menu hides these file names. Only the name is matched, never the parent directory. Shared by every workspace.",
  "还没有规则——内置的忽略目录（.git、node_modules 这类）不在这里，它们一直生效": "No rules yet — the built-in ignored directories (.git, node_modules and the like) are not listed here; they always apply",
  ...
  "选中一个项目之后，这里还能给那个工作区单独加规则。": "Once a project is selected, you can add rules just for that workspace here.",
