...
> 2026-08-16 起 · 分支 `acp` · 状态：**A1–A5 与路线 C 已完成；路线 B 本体待做**
...
| **B1·B** | 把我们自己的工具作为 MCP 递给它 | ⬜ **下一个里程碑** |

协议从 7.0 走到 **7.3**（会话种类 `acp` → 权限询问 → 会话开关）。

### 路线 B 本体还差什么

协议那一侧是现成的：`newSession` 的 `mcpServers` 字段。**缺的是一条路**——
那台 MCP 服务器是被 **agent** 拉起的独立进程，它得能回到 DAWN 才拿得到
内核、技能、账本。需要：

1. 主进程开一个本地 socket（Windows 上是命名管道——见 `launch.ts` 的文件头，
   那一整段跨平台的坑在这里会再来一遍）；
2. `scripts/dawn-mcp-server.mjs`：一台真的 MCP 服务器，把工具调用转到那个 socket；
3. 一次性令牌走 `env` 递给它（**不能落盘**）；
4. 每一次 `dawn_*` 调用落一条 Run，`parent_run_id` 挂在那一轮 ACP 回合上。

第 4 条才是路线 B 与 wisp 的实质区别：**它不是能力裁剪，是账本延伸**。
