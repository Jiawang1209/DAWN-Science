...
- **What**:
  ...
  - **顺带补上「设置」那一屏缺的等待**：`visual.spec.ts` 此前不等分类列上那四个
    ...
    - 「插件」「MCP 服务器」这两个数**是这条分支今天才加的**（`41dbf13`，Task 1）。
      基线存于 `aeb7616`，而 `git merge-base --is-ancestor 41dbf13 aeb7616` 为假——
      基线里那两行没有数字是**对的**，它们那时还不存在。
    - 「Skills」5 → 6 **不是渲染变了，是内容真的多了一个**：`a74ab7a`（main，
      2026-09-15）加了 `skills/analyze-like-case/`。**这张基线因此整整一天都在
      掩盖一次真实的内容变更**——逐像素对比把它吞掉了，没有任何人收到消息。
