...
| 8 | `runtime/native.ts` | 用 agent loop 驱动任意 API 端点（DeepSeek 等） | 🔧 **入口**：`pi-coding-agent` 的 `createAgentSession()`（第三层）<br>🔧 **能力来自**：`pi-agent-core`（loop · harness · 压缩 · skills · bash/read/write/edit）+ `pi-coding-agent`（grep/find/ls · model-registry · auth-storage · 扩展）+ `pi-ai`（39 provider） | 🔧 **pi**（直接 import）+ 🔧 **Buzz** `buzz-agent`（会话与上下文管理的完整参考实现） |

> **#8 的技术栈栏 2026-08-08 两次改写，两次都是被指出的。**
>
> **第一次**：原文写「`pi-agent-core` · `pi-ai`」——**那不构成决策**，`new Agent({tools: []})`
> 也满足它，而实现正是落成了这个，agent 一个工具都没有。
>
> **第二次**：改成「坐第三层 `pi-coding-agent`」后，作者指出 agent loop / harness / 压缩 /
> skills **本来就是 `pi-agent-core` 提供的**，不该记在 coding-agent 名下。**这是对的**——
> 我把「谁提供能力」和「从哪个入口进」混成了一句，而这恰恰是分层纪律要防的含混。
> **坐第三层不是放弃 agent-core，是连它一起拿到。**
>
> **本表的「技术栈」栏今后必须分开写「入口」与「能力来源」，并具体到导出符号。**
