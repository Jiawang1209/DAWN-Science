...
| **入口** | 界面上加得进来一个适配器（2026-08-19，见下） | ✅ `2491336` |
...
## 缺的最后一块：**界面上根本加不进来**（2026-08-19）

作者：*「你现在要在选择模型的地方加上我们之前开发 ACP 的东西，
否则岂不是白开发了。」*

他说得对，而且比字面更严重一点。上面那张表每一格都是 ✅，
但**默认 `providers.yaml` 里一个 `kind: acp` 都没有，界面上也没有任何地方能加**。
唯一的路是自己打开那个 yaml 手写一段——

> **于是这一整套对使用者而言等于不存在。**

这与 2026-08-10 kimi 那件事是同一个形状。当时写在 `config/writer.ts` 文件头的那句话
一字不改地适用于今天：*「让人打开一个 yaml 手写一段，本身就是这个应用没做完。」*

### 入口放哪（作者 2026-08-19 拍板）

三个候选摆过：设置里加一格 + 选择器里出现 / 直接做进模型选择器 / 两处都有。
**作者选了第一个。**

理由站得住：他说的是「选择模型的地方」，而缺的其实是**「怎么把它加进来」**——
那是**配置**，不是**选择**。塞进选择器的菜单里，那个菜单就同时是
「挑一个」和「造一个」两件事；而配置在这个应用里已经有一个家（设置 → 模型服务就在隔壁）。

**「加完立刻出现在选择器里」是这个选择成立的前提**，所以后端加完之后
原地更新那份被多处持有的 `registry.agents`——只写文件不更新内存的话，
界面会说「已添加」而选择器要等重启才有它，**那是一句半真的话**。

### 与 native 那条的差别只有一处

**它没有 provider / model。** ACP 的模型由适配器自己广播（`session/new` 之后那串
`models`，见上面真机那一节逐条记着的差异），我们只知道「用哪条命令把它拉起来」。
所以写进 yaml 的那一段**不写 `model:`**——写一个猜出来的值，
会在换模型时与适配器广播的那一串打架。

### 预置两条，外加一条自定义

预置的是**真机那一轮跑通过的那两条命令**（`@agentclientprotocol/codex-acp` /
`@zed-industries/claude-code-acp`），不是「大概是这么用的」。
它们走 `npx`，**要求机器上有 Node——这句话写在按钮旁边**，
而不是等人点下去看一串 ENOENT。

自定义那条**刻意不做 shell 解析**：参数按空白切，带空格的路径请直接改 yaml。
**半个 shell 解析器比没有更坏**——它会在某些引号下悄悄切错。

### 加得进去就得删得掉

`removeAgent` 与它同一批做。「只能加不能删」意味着加错一次之后人又得回去
打开那个 yaml，而「不必打开那个 yaml」正是这条路存在的全部理由。
**最后一个不给删**：`agents:` 空了配置读不回来，应用下次起不来。

### 判据

`e2e/acp-setup.spec.ts`。它与 `acp-agent.spec.ts` 的分工要写清楚：
后者拿一份**预先写好的 yaml** 起，验的是 ACP 协议那一段；
**前者的起点是「配置里一个 acp 都没有」**，全靠界面加进去，
然后真的用它聊一段——中间隔着协议、config writer、内存里那份 registry 的
原地更新、以及模型选择器的重取，**每一环都能单独坏掉而单测全绿**。
